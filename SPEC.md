...
> **At a glance** · Five tables. `users` (Devise auth, `jti` for JWT revocation), `credentials` (WebAuthn passkeys — one row per enrolled authenticator), `push_subscriptions` (Web Push — one row per subscribed browser), `applications` (the core FSM entity — `status`, plus `resume`/`cover_letter` as `bytea`), and `timeline_entries` (append-only audit log, one row per status change).
...
`FollowUpReminderJob` already claims each reminder through the `timeline_entries` idempotency key (§ Background jobs), and the claimed `won` set drives the mailer. Push adds **no second claim**: the same set now also enqueues one `PushDigestJob.perform_later(user, application_ids)` beside the mailer's `deliver_later`. The decoupling is the same as the mailer's and for the same reason — a push-service failure retries the push job alone, without re-running the reminder scan or double-sending the email, and the timeline claim remains the one exactly-once anchor for both channels.

`PushDigestJob` composes the notification from the same facts the email renders — the mailer's subject rule reused as the title (*"Follow up on your Mercari application"* / *"3 follow-ups due today"*), the deep link `/applications/:id` for a single reminder and `/dashboard` for several — and sends it to every subscription the user holds, with a **24-hour TTL**: a digest that could not be delivered today is superseded by tomorrow's, not queued behind it. Copy is English-only, matching the mailer it mirrors (§ Mail). Delivery failures split three ways: the push service's `404`/`410` (`WebPush::ExpiredSubscription` / `InvalidSubscription`) means the browser revoked the subscription, and the row is **destroyed** — self-pruning, since browsers do not reliably fire `pushsubscriptionchange`; any other response error is logged and skipped (one dead endpoint must not cost the user's other devices their notification); anything else propagates into Solid Queue's retry.
