...
- **Uploads**: size is checked from multipart metadata *before* `.read`, so an oversized file never enters memory. Then the 1 MB model cap (`Application::MAX_FILE_SIZE`; `web/`'s own `MAX_FILE_BYTES` in `app/lib/files.ts` is the client-side twin of the same number, and the two are kept equal by hand), then a PDF magic-byte check (`%PDF`), which renaming cannot spoof. The frontend's `accept=".pdf"` is UX only.
