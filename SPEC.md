...
**Backups now run locally and push out, rather than being pulled from outside.** Railway's Hobby plan had no managed backups, so a GitHub Actions runner reached out to Railway's public Postgres nightly; a home Postgres behind a Tunnel has no route a GitHub-hosted runner can reach, and shouldn't be given one. Instead a systemd timer on the host runs the same fingerprint-and-dump script against `localhost`'s Postgres at the same 05:15 JST cadence, and `git push`es the gzipped result to the same private [`karirkalyan-backups`](https://github.com/chairulakmal/karirkalyan-backups) repo, on the same **60-day retention** four sentences on `/privacy` (including the erasure promise) name explicitly. Only which machine computes the dump changed; the destination, the retention, and the restore drill did not.
