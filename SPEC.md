...
- **CI runs on every pull request and on pushes to `main`, including Playwright.** Every action is pinned to a commit SHA with its version in a comment, and every workflow's token is `contents: read`. Dependabot is off, so the pins are raised by hand. `dependency-audit` runs `bundler-audit` and `npm audit --omit=dev --audit-level=high` every Monday, because under the freeze the lockfiles rarely change and the per-PR audits rarely run.
...
> **At a glance** · Self-hosted since 2026-08-20 on the author's own mini PC, moved off Railway to close a ~$10/month cost for an app with one user; the mini PC's low power draw keeps the electricity cost small. It is a transition, not the final host: the `2.0.0` rewrite onto Cloudflare + Hono + Nuxt is the target $0 hosting and replaces this stack entirely (`TODO.md`). Four containers (`postgres`, `api`, `web`, `cloudflared`) on one internal Docker network, driven by the root `docker-compose.prod.yml`. No container publishes a host port: `cloudflared` is the only ingress, reaching `api` and `web` by Docker service name, so the app is unreachable however the host firewall is configured. The domain (`kk.chairulakmal.com`), `FRONTEND_URL`, and every env var below are unchanged from the Railway era; only who runs the containers and how traffic reaches them moved. The Railway-era version of this section, kept for the operational lessons it recorded, is archived in `notes/HISTORY.md`. The host ran Docker Desktop for Linux until 2026-09-01 and runs native Docker Engine now; two properties in § Network exposure changed with that move (the `ufw` bypass is now real, and bind-mount file ownership now reaches the container unchanged).

| Service | Image | Role |
|---|---|---|
| `postgres` | `postgres:18.6` | Single instance, named volume (`postgres_data`, mounted at `/var/lib/postgresql`: `postgres:18` moved `PGDATA`, see § Local development) |
...
- **`cloudflared` and `postgres` are pinned to real tags, never `:latest` or a bare major.** A bare `postgres:18` never updated here: `build --pull` re-resolves only Dockerfile `FROM` lines, and `up` does not pull an image it already has, so production stayed on the first 18.x it pulled. Changing a pinned tag is what makes `up` pull. Dev and CI track the major. `docker compose build` only rebuilds services with a `build:` key, so an unpinned `cloudflared` survives ordinary redeploys unchanged. It would then silently pull whatever `latest` resolves to after a `docker system prune`, after disk pressure, or after a move to new hardware, with no version recorded and no changelog read. Raise the tag deliberately, when there is a reason to.
...
- `api` reads the root `.env` (gitignored; `.env.prod.example` is the template): `DB_USERNAME` / `DB_PASSWORD` / `DB_NAME` (the app's own **non-superuser** role, `karirkalyan_app`; the same three reach the `postgres` service as `APP_DB_USER` / `APP_DB_PASSWORD` / `POSTGRES_DB` for its healthcheck and first-boot init script, so the halves cannot drift; `DB_HOST` is set by compose, being a property of the topology), `DEVISE_JWT_SECRET_KEY`, `SECRET_KEY_BASE`, `FRONTEND_URL`, `SOLID_QUEUE_IN_PUMA` (**required**: without it no job ever runs), `HONEYBADGER_API_KEY`, `ANTHROPIC_API_KEY`, `SMTP_*`, `MAILER_FROM`, and `VAPID_PUBLIC_KEY` + `VAPID_PRIVATE_KEY` (**optional**).
...
- `postgres` reads its own `.env.postgres` (`.env.postgres.example`) for the cluster superuser the image bootstraps, `POSTGRES_USER` / `POSTGRES_PASSWORD`. No other container reads that file: `api` connects as the plain role above, and the backups runner reaches Postgres over the container's Unix socket, which needs no password. The superuser exists for `docker compose exec postgres psql` and nothing else.
- Compose still reads the root `.env` automatically to resolve `${DB_*}` inside `docker-compose.prod.yml`, independent of any `env_file:`.
- **`api` is not a database superuser** (merged 2026-09-17, run on production 2026-09-24). `POSTGRES_USER` is always the bootstrap superuser, and until then it was also `DB_USERNAME`, so an injection or a Rails RCE had `COPY ... TO PROGRAM` inside the database container. `karirkalyan_app` owns `api_production` and every table and sequence in it, which is what migrations need, and nothing else: no `SUPERUSER`, `CREATEDB` or `CREATEROLE`. The `pgcrypto` extension stays owned by the superuser (extensions have no `OWNER TO`; the app only calls its functions, and `pgcrypto` is a trusted extension, so a schema load on a fresh database can still `CREATE EXTENSION` it as the database owner). On a fresh volume `postgres/initdb/10-app-role.sh` creates the role from `APP_DB_*`; on the live database it was created by hand following `notes/OPS.md` § Move the app off the superuser, with `postgres/transfer-ownership.sql` walking the catalog because `REASSIGN OWNED` refuses the bootstrap superuser.
...
- **Deploys are manual: `bin/deploy`.** A merge to `main` is no longer itself a deploy. The script refuses any branch but `main`, runs `git pull --ff-only`, tags the images the running `api` and `web` containers use as `rollback` (by image ID, because a failed deploy can leave `:latest` on code that never ran), builds new images tagged with the commit (`DEPLOY_TAG`, also passed to Honeybadger as the revision), runs `up -d --wait`, and then requests both public hostnames. If any of that fails, it brings the `rollback` images back up. After success it keeps only the current and `rollback` tags and prunes dangling images. **A rollback restores code, never the schema**, so a migration must keep the previous release working. Migrations need no separate step, since `bin/docker-entrypoint` runs `db:prepare` on every boot of a `rails server` command.
- **`bin/deploy` refuses a dirty working tree.** `docker compose build` reads the tree, not the commit, so an uncommitted edit would otherwise ship as production with no trace in git. The script exits before `git pull` if `git status --porcelain` prints anything.
- **Base layers refresh only when told to.** `--pull` re-resolves the `FROM` tags on every deploy, but the `apt-get install` layers in both Dockerfiles are cache hits until their instruction text changes, so OS patches in the running images date from the last cache miss. `NO_CACHE=1 bin/deploy` forces the full rebuild; run it about monthly. `ruby:3.4.9-slim` is pinned to a patch version, so a Ruby security release needs the `ARG` edited as well.
...
- **No service publishes a host port.** All four containers sit on the single `internal` bridge, `cloudflared` dials *out*, and the backups runner reaches Postgres through `docker exec` rather than TCP. Measured, not assumed: `ss -tlpn` filtered to non-loopback addresses returns **nothing at all**. That measurement was false for a while and nobody noticed: the shared development Postgres (`devdb`, its own compose project outside this repo) published `5432:5432`, which on native Docker Engine is a LAN-reachable superuser cluster past `ufw`. Re-measured 2026-09-17, it is bound to `127.0.0.1` and the statement holds again. The lesson is that this property is per host, not per compose file, so a re-audit means running `ss`, not reading this file.
- **`ufw` is active**: default deny incoming, allow outgoing, routed disabled. `openssh-server` is not installed.
- **The usual "Docker punches a hole through `ufw`" caveat now applies to this host, and the first rule above is what makes it harmless.** Stock Docker Engine inserts `iptables` DNAT rules that bypass the `INPUT` chain `ufw` filters on. Until 2026-09-01 this machine ran Docker Desktop for Linux, whose engine lived in a VM, so the bypass did not exist: `iptables -t nat -L DOCKER` reported no such chain and there was no `/var/run/docker.sock` on the host. Native Docker Engine runs now, so both exist and the bypass is real. It changes nothing for this stack because no service publishes a host port, so Docker inserts no DNAT rule to bypass with. The bypass would matter the moment any service gains a `ports:` mapping: that port would be reachable from the LAN past `ufw`, and only the no-published-ports rule would still be holding the line.
- **Containers run unprivileged**: `api` and `web` as uid 1000, `cloudflared` as uid 1000 (set by `user:` in the compose file; the image's own default is 65532, see the mode-600 note below), `postgres`'s postmaster as `postgres`. None is `privileged`. Since 2026-09-17 every service drops **all** capabilities and sets `no-new-privileges`, via one `x-hardening` anchor in the compose file. Since 2026-09-24 the same anchor sets `pids_limit: 256`, so a fork bomb after a compromise cannot exhaust the host's process table (measured: `api` 31 tasks, `postgres` 14 plus one per connection). `postgres` alone adds back the five its root entrypoint needs to fix ownership of the data directory and drop to `postgres` (`CHOWN`, `DAC_OVERRIDE`, `FOWNER`, `SETGID`, `SETUID`); the set was test-booted against `postgres:18` before it shipped. A `postgres` healthcheck failure after a change to that list is the first thing to suspect.
- **Container logs are capped at 30 MB each** (`json-file`, `max-size` 10m, `max-file` 3, in the same anchor). The host has no `/etc/docker/daemon.json`, so without the cap the driver is unbounded, and Rails logs every request at `info`; a full disk on this host takes Postgres down with it.
- **Every file holding a production secret is mode 600.** Four were **664** until the 2026-08-20 audit, and all four were created during the move to self-hosting. The older secrets already followed the convention. The new deploy files did not inherit it, because nothing enforces the mode.
- **The mode-600 files reach the container with their host ownership intact, and `cloudflared` runs as `user: "1000:1000"` to match.** Under Docker Desktop for Linux the file-sharing layer remapped bind-mount UIDs, so a container user could read a host file it did not own, and the mode defended only against other host users. Native Docker Engine (since 2026-09-01) passes host ownership straight through. The `cloudflare/cloudflared` image runs as uid 65532 by default, so on the cutover it could no longer read `cloudflared/config.yml` and the tunnel credentials JSON, both mode 600 and owned by uid 1000, and the container crash-looped. Setting `user: "1000:1000"` on the `cloudflared` service matches the file owner and keeps the files at mode 600 rather than widening them. `cloudflared` needs no privileged port: the tunnel dials out.
- **The self-hosted runner's blast radius is bounded by one fact**: `karirkalyan-backups` is private and this repo's own workflows all run on `ubuntu-latest`. A self-hosted runner attached to a *public* repo is the classic RCE hole, because a fork's PR can propose the workflow that runs on it. What remains is not a network attack: push access to the private backups repo is code execution on this machine, which is GitHub account hygiene, and why the runner is registered to that one repo rather than to the account. The runner service runs as the operator's own user, which is the only member of the `docker` group, so anything the backups workflows execute, including the third-party actions they `uses:`, holds root-equivalent access to this host and can read every secret file above. Those actions are therefore pinned by commit SHA, not by major tag (since 2026-09-17), so a moved tag cannot change what runs here.

**What the audits did not cover**, recorded so the gap is not mistaken for a clean result: Cloudflare's own edge configuration, meaning WAF rules, rate limiting and Bot Fight Mode. That is the one layer in front of the origin that lives entirely outside this repo. `TODO.md` holds it, along with the one hardening step the 2026-09-17 container review left open: no container runs a read-only root filesystem.
...
- `psql` / `pg_dump` run via **`docker exec`** into the `postgres` container, which drops the per-run `apt-get` that installed a version-matched client and the `DATABASE_URL` secret itself (a local `docker exec` authenticates over the container's Unix socket). It runs as the superuser, which is why moving `api` to its own role changed nothing in that repo.
...
Operator runbook (destroying a user, resetting the demo account, moving the app off the superuser): [`notes/OPS.md`](notes/OPS.md). Production lessons from the Railway era, kept so they are not relearned: [`notes/HISTORY.md`](notes/HISTORY.md).
