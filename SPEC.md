...
The **Japanese-level dropdown** (`v1.8.0`) is a fourth filter control and deliberately the dumbest of them: its options are the fixed `Application::JAPANESE_LEVELS` taxonomy plus "All", with **no counts and no faceting**. Counts would need the level in the `facets` payload, and that payload's next reshape is already spoken for — the `v1.10.0` stat-cards item folds status in and bumps `STATS_CACHE_VERSION` once; changing the shape twice for one filter's garnish is the two-migrations-of-the-same-shape mistake that item exists to avoid. A level with no rows simply matches nothing, which the existing "no matches" state already renders honestly.
