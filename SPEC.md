...
- **Negative tracking is a Latin display device**, so a `:lang(ja)` rule resets it to `0` on plain headings (kanji and kana are drawn to full-width boxes), excluding `.kk-label` and `.kk-display`, which set their own. The exclusion list survived the typeface change unchanged; only the comment naming the old serif had to move.
