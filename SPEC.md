...
- **Fraunces is an optical-size variable font, and `opsz` is not a size**: it is how the letterforms are drawn *for* a size. The `h1, h2, h3` rule sets `opsz 36`, a heading cut whose thin joins go weak past ~60px. The homepage hero therefore uses `.kk-display` (`opsz 144`, the wordmark's cut, with tracking pulled in). It is the only display-scale type on the site. That heading rule also pulls in negative letter-spacing, a Latin display device; a `:lang(ja)` rule resets it to `0` on plain serif headings (kanji and kana are drawn to full-width boxes), excluding `.kk-label` and `.kk-display`, which set their own tracking.
