...
Last synced against the code: **2026-08-21**. Newest first. **The `api` container's memory limit rises from 512m to 768m**, after a measurement found it pinned against its 512 MiB ceiling rather than sitting under it: `anon` 493 MiB of real Ruby heap, every page of cache evicted, and `memory.events`' `max` counter at 92, while `oom_kill` and the restart count both stayed 0, so nothing had ever surfaced it. § Deployment now records the limits and why only the two app containers carry one, which this file had never documented at all. Config only: no application code changed, and the running container took the new ceiling via `docker update` without a recreate. Before it, **A network security audit ran against the running stack and the machine hosting it**, and § Network exposure is the new section recording what it verified: the origin binds no TCP service to any routable interface, `ufw`'s default-deny genuinely covers Docker here (Docker Desktop installs no host NAT chains, so the stock-Engine `ufw` bypass does not apply, which is a property of the runtime and not of this app), and every container runs unprivileged. It changed no application code: the app-layer controls this file already documents, the `forwarded_priority` pin, the throttle discriminators, host authorization, the CSP nonce, the httpOnly session cookie, all held under review. What it did fix was four secret files left at mode **664** by the cutover, now 600, and what it left open is in `TODO.md`, along with the one layer it could not audit from inside this repo: Cloudflare's own edge configuration. Before it, **Production moves off Railway to a self-hosted Docker stack behind a Cloudflare Tunnel**, run on the author's own machine: § Deployment (Docker + Cloudflare Tunnel) is rewritten around it, and the Railway-era version of that section is archived in `notes/HISTORY.md` rather than deleted. The domain, `FRONTEND_URL`, and every env var this file already documented stay the same; what changes is who runs the containers and how the origin is reached (`cloudflared`, not a public listener). The motive is cost, not architecture: Railway ran close to $10/month for a single-user app, and this is a bridge until `2.0.0`'s Cloudflare-native rewrite, not a redesign of anything `api/` or `web/` does. **DNS cutover completed the same day**, after a parallel-then-switch verification window against temporary hostnames and a real-data restore drill from the last Railway-sourced backup (`TODO.md`): `kk.chairulakmal.com` and `kk-api.chairulakmal.com` now resolve to this machine's tunnel, and Railway serves nothing. **§ Backups now names the actual mechanism**: a GitHub Actions self-hosted runner on this same machine, registered to the private `karirkalyan-backups` repo, running its existing `check.yml`/`backup.yml` (same cron, same fingerprint script, same artifact retention) via `docker exec` into the `postgres` container instead of a `DATABASE_URL` over TCP to Railway's public proxy; the earlier draft of this section described a systemd-timer-and-custom-script design that was never built. **The dashboard grows a fourth stat card, the screening success rate**: the percent of applied applications the company took past the resume screen, which is `response_rate` without the rejections, so the gap between the two separates a targeting problem from a resume problem (§ The dashboard payload). It is a capability, so it is the feature freeze's third recorded exception and `TODO.md` § The rule carries the argument both ways; `STATS_CACHE_VERSION` goes to 8. Beside it, **the dashboard is reordered and the Upcoming agenda is windowed to seven days**, and § Dashboard layout is new, the section that now owns every display rule on that page. The applications list rises to second, under the agenda; the ghost-risk card drops below it (the rows it names are already flagged inside the list, so the nudge does not ride on its position) and Your data drops to last. Upcoming shows the next 7 days plus anything overdue, with the residence clock exempt because a 63-day CoE lead time cannot be served by a 7-day warning; anything further out folds behind the "Show more" toggle it already had. Before it, **both action cards began capping their visible rows at three**, each re-ranking its own three for the read the section is for. The layout rules are display-only in `web/`: the server's payload, its orderings (longest-silence-first, chronological) and `AGENDA_LIMIT` are unchanged on the wire. The one `api/` change beside them is the **demo seed, whose dates are now anchored on every run** and placed on both sides of the new window, so the walkthrough always opens with an agenda on it (§ `Demo::ResetService`). **`v1.11.1` (2026-07-28) is the last tag of `v1`**, so the newest work below is deployed and untagged (§ Versioning & releases). Before it, the one thing in this file the feature freeze does not explain: **dashboard pins** (§ Pinned applications), up to three applications hoisted to the top of the list, held in `localStorage` and nowhere else. It is a capability, so it fails `TODO.md`'s admission test on its face and was built anyway, as a decision the author took with the trade in front of him; `TODO.md` § The rule records it as a named exception rather than pretending the test passed. It touches no Rails file, no schema and no endpoint. Before it, a pair of fixes that turned out to be one bug seen from two ends: the dashboard's `response_rate` and `ghost_rate` inferred "was applied to" from the *current* status, so archiving a `wishlist` row moved it into the denominator it could never contribute a numerator to (§ Dashboard), and the FSM had offered that row a second way out to begin with, so `withdrawn` now starts at `applied` and the pre-application stages exit through `archived` alone (§ State machine). A review of the same branch found two more, both places where a rule this file already states was not being kept: the account archive rode `Application#as_json` and so shipped without `posting_snapshot`, the one column no other export carries (§ Exports), and `PATCH /me` answered validation failures off the `{ error, code }` envelope every other endpoint uses (§ API contract). Before them, two more fixes: the prefill extractor read a residents-of-Japan-only posting as `sponsorship: "unavailable"`, asserting a refusal to sponsor that no posting had stated (§ `UrlPrefillService`); and ghost risk counted calendar days, so every threshold silently shrank across Golden Week, Obon and the New Year, which is when it should have been most patient. Fixing the second took the derived p90 with it: the thresholds are now fixed counts of business days, and the payload lost `basis` and `sample_sizes` (§ `Applications::GhostRiskQuery`). Then **`v1.11.1`**, the bug-fix patch. Its headline is that **the list's `?q=` search never reached the API**: the BFF route handler re-enumerated the query params by hand and was never taught about `q`, so a typed search silently returned unfiltered rows while every surface reported success (the allowlist now lives once, in `app/lib/list-params.ts`; spec and code disagreed and the **code** was the bug, so § Query layer is unchanged).
...
- [Deployment (Docker + Cloudflare Tunnel)](#deployment-docker--cloudflare-tunnel) · [Network exposure](#network-exposure-audited-2026-08-20) · [Backups](#backups) · [Production lessons](#production-lessons-recorded-so-they-are-not-relearned)
...
- **Open registration means strangers' resumes.** A resume is close to the most PII-dense document a person owns: legal name, address, phone, employment history, sometimes a photo and a date of birth. This app stores it as `bytea` in a single self-hosted Postgres (§ Deployment), whose only backup is a nightly `pg_dump` run by a GitHub Actions self-hosted runner on that same machine (§ Backups). That is an honest arrangement for *my* resume. Accepting yours would make it a custodial promise I have not built the machinery to keep.
...
The `(application_id, created_at)` composite **replaces** a bare `application_id` index, which it covers as a prefix, so it is a widening, not an extra index. It exists because every read of this table is per-application in time order: the detail page's timeline, and the `MAX(created_at)` per-application lookups behind the ghost-risk query and the board's `days_in_stage`. It was introduced for a `LAG(created_at) OVER (PARTITION BY application_id ORDER BY created_at)` window function that the 2026-08-03 ghost-risk fix removed with the derived threshold; the index earns its keep on the correlated `MAX` subqueries that replaced it, which want the same ordering.

**`to_status` is indexed for exactly one value, and partially.** `index_timeline_entries_on_offer` covers `(application_id, created_at) WHERE to_status = 'offer'`, which is the precise shape the dashboard's first-offer `DISTINCT ON` reads (§ Dashboard): the predicate supplies the filter and the column order supplies the sort, so the index serves the whole subquery rather than half of it. It shipped 2026-08-05, alongside the fix that scoped that subquery to the requesting user; before it, the read was a sequential scan and sort of the entire table on an endpoint the stats cache sends traffic through after any edit to any application.

A **plain** index on `to_status` is still deliberately absent, and the partial one is why it can stay that way. `'offer'` is one of thirteen states and the only one any query asks for by value; indexing the other twelve would pay write amplification on every transition to buy nothing. This is the same personal-scale reasoning that leaves `japanese_level` unindexed, with the one exception carved exactly where a measured cost was.
...
**`note` is the interview-stage note (`v1.10.0`), and the column already existed.** `TODO.md` scoped "interview stage notes" as a new `timeline_entries` column; the code already had one, carrying the revival reason a `→ applied` re-open records. So the release added no migration, only the affordance the column was waiting for: advancing into an interview stage (`phone_screen`, `technical`, `final_round`, `offer`) now offers an **optional** note ("who you met, what they asked"), recorded as that transition's `note` and shown on the timeline where revival reasons already are. The prompt is the detail page's alone; the board's card menu stays a one-gesture move, because the note is optional and a quick drag should not grow a text box. The set of stages that offer it is `web/`'s `STAGE_NOTE_STATES`, an affordance set beside `CONFIRM_REQUIRED` and `canRevive`, not an FSM copy that could authorize a move (§ Board view records why those sets are safe). **`v1.11.0` extends the same optional textarea to the `CONFIRM_REQUIRED` closing moves** (`rejected`, `accepted`, `declined`, `withdrawn`, `archived`), so a rejection's feedback or an offer's terms attach to that exact transition rather than to the single `applications.notes` blob. `TransitionService` already accepted a `note` on any move, so this is affordance only, no migration. As with the stage note, the prompt is the detail page's alone: the board's card menu and its drag stay promptless, and a past transition's note stays uneditable (the note is written at transition time, so this helps future closes only).
...
- `withdrawn`: candidate-initiated, *before* any decision, and only once there is an application to withdraw
...
**`withdrawn` starts at `applied`: `wishlist` and `draft` reach it through `archived` instead.** The two rows that let a pre-application row be withdrawn were removed, so the pre-application stages now have exactly one exit each (`archived`) beside their forward move. Two reasons, and the second is why it is a fix rather than a preference. Withdrawing from something you never applied to is not a candidate-side outcome, it is deciding not to pursue a lead, which is what `archived` already means ("remove clutter without deleting history"); two verbs for one act invites the user to record the same decision two ways and then asks every stat to reconcile them. And the ambiguity had already leaked: § Dashboard's `response_rate` and `ghost_rate` inferred "was applied to" from the *current* status, so a wishlist row withdrawn or archived stopped looking pre-application and silently entered the denominator. That inference is gone (it reads `applied_at` now), but the FSM should not have offered the move in the first place.

This removes a legal move, so it is the one shape of FSM change that can invalidate stored history: rows already sitting in `withdrawn` after a `wishlist` or `draft` exit stay exactly as they are, and their revival edge back to `applied` is untouched. `assert_transition!` governs new moves only, so nothing needs migrating, and nothing is.
...
**Nothing in `db/seeds.rb` carries a calendar date, and the reason is the dashboard.** The two sections that answer "what do I do today?" are both date-driven: the Upcoming agenda shows the week ahead (§ Dashboard layout) and the ghost-risk card counts business days of silence. A fixture written with fixed dates passes every test, seeds cleanly, and then shows a visitor two empty panels a month later, which for the one account every visitor sees is the walkthrough gone. So the demo's dates are anchored to the run, at a pinned hour in app time (an interview reads 15:00, not whatever minute the seed fired), and deliberately placed on both sides of the 7-day window: a follow-up two days overdue, an interview in two days, an offer deadline in three, a wishlist reminder twelve days out that folds behind "Show more", and the demo user's own residence expiry 80 days out, inside the residence window and still clear of the CoE lead time. The overdue one is load-bearing: the window has no lower bound, so that row keeps the section populated however long ago a local database was seeded. `follow_up_at` and `interview_at` are therefore rewritten on **every** run rather than only the run that creates the row, which is safe here because a reset destroys the account first and re-seeding is allowed to overwrite what a visitor did. `spec/services/demo/reset_service_spec.rb` asserts the property (a reset leaves both sections populated) rather than the dates. That overdue follow-up is also why `FollowUpReminderJob` skips this account outright (§ `FollowUpReminderJob`).
...
`Exports::InterviewCalendar` builds the `.ics` (`GET /applications/:id/interview`): a hand-written RFC 5545 VEVENT, folded on **octet** boundaries rather than character ones, because a fold landing mid-multibyte-character is malformed and `company`/`role` are routinely Japanese here. Its `escape` handles the four TEXT specials, and **all three line-break forms collapse to the escaped `\n` literal**: `\r\n`, a bare `\n`, and, since 2026-07-28, a bare `\r`. That last one used to survive into the content line, and nothing validates the *format* of `company`, `role`, or `url` (they are `presence: true` only), so a CR planted in a row reached the file raw and a parser that still treats a lone CR as a line terminator would read what followed as real calendar properties (`BEGIN:VALARM`, `ATTENDEE`, `URL`). CRLF was safe only by accident: the `\n` rule caught the LF and left a stray CR that happened to break the injected property name. The same class of bug as the CSV escaping above, and it took the same kind of fix: escape at the boundary, not at the call sites.
...
**`q` is the free-text filter (`v1.11.0`):** a case-insensitive `ILIKE` substring over `company`, `role`, and `notes` at once, the one filter the structured params cannot express because the match is partial. It inherits the class's whole contract: blank or whitespace-only is ignored like `company`/`source`, the `%`/`_` wildcards inside the term are escaped to literals, and it `AND`s against every other filter. Being a server param is the point, since the list paginates by cursor: a client-side scan would only see the pages "load more" has fetched, so a search that silently missed matches on unloaded pages would be worse than none.
...
**The envelope is a rule about every error, not a convention most of them follow.** `PATCH /me` used to render `{ errors: … }` straight from `ActiveModel::Errors`, the one endpoint in the API with no `error` sentence, no `code`, and no `details`; the table above was right and the code was the bug, so the controller now calls the same `render_validation_failed` the other three writers do. Nothing caught it because a wrong body under a right status is invisible to a spec that asserts only the status, which is why the `/me` spec now asserts the envelope. The rule for a new endpoint: an error leaves through `ErrorRendering`, never through a hand-built hash.

Codes are append-only: renaming or removing one is a breaking change to `web/`'s message catalog, adding one is not (unknown codes fall back to status-keyed copy). This table claims to be exhaustive, so it is worth recording how it stopped being so: the three `talking_points_*` codes shipped with `v1.10.0` and reached `web/`'s catalogs in both languages, and only this hand-written table lagged. The catalogs are the surface that gets exercised; this one has to be checked deliberately. `/up` also returns `503` when Postgres is down, but it is a health probe with its own body shape (`{ status, checks }`), not part of this error contract; and for the same reason it carries no OpenAPI path. It is infrastructure, not API; its absence from `swagger.yaml` is deliberate, not a missing rswag spec.
...
`GhostRiskCard` re-ranks and caps `at_risk` for display only; the server's longest-silence ordering is unchanged on the wire (§ Dashboard layout, which owns every display rule on this page).
...
**`upcoming` is the Upcoming agenda (`v1.11.0`),** the dated commitments merged into one chronological list: follow-ups and future interviews across the active applications, plus the user's `residence_expires_on` when it falls within `AGENDA_RESIDENCE_WINDOW_DAYS` (90). Each item carries a `type` tag (`follow_up` / `interview` / `residence`) and, except a `residence` item, the application it deep-links to; the list is capped at `AGENDA_LIMIT` (8). It is gated on `ACTIVE_STATES` for the same reason `isOverdue` is (a dated fact on a closed application is not actionable), and interviews are future-only so a finished one drops off. `web/` renders it as a dashboard section, never an `/upcoming` route, the same call the stat cards made against a dedicated page. **Every item serialises in Tokyo, the residence row included** (`v1.11.1`): it is built from a `date` column, and `Date#to_time` resolves in the container's system zone (UTC on Railway), which had that one row emitting `+00:00` beside every other item's `+09:00`; `in_time_zone` is what makes the zone the app's rather than the host's, and `dashboard_spec` asserts the offset now, which it never did.

`UpcomingAgenda` re-ranks the array, filters it to a 7-day window and caps it, all for display only; the server's chronological ordering and `AGENDA_LIMIT` are unchanged on the wire (§ Dashboard layout).
...
- **`STATS_CACHE_VERSION`**: bump it whenever the payload's shape changes **or the way any figure in it is computed does**. A data-derived key cannot see a deploy, so unchanged rows keep serving the old payload to new code. The second half of that rule is not padding: it was written as "shape" alone until `v6`, which changed no shape at all (the outcome-rate denominator moved from `status` to `applied_at`) and so was exactly the case the narrower wording would have talked someone out of bumping for. A recomputation nobody bumps for is a wrong number served until the rows happen to move.
...
`user` and `upcoming` are merged in *outside* the cached block. Both are cheap reads, and keying application stats on a user record would be a category error; `upcoming` is out of the cache additionally because it reads the user's residence field, which the stats key (application count + `MAX(updated_at)`) cannot see, so caching it could serve a stale clock.
...
**The dashboard defaults to the active applications, not to every status.** A bare `/dashboard` renders the `active_states` set (the seven in-play stages, § API contract), because the day-to-day question a tracker answers is "what is still in play", not "everything I have ever touched". This is a `web/` presentation default layered on top of `ListQuery`, whose wire contract is unchanged: absent `status` still means unfiltered *there*, which is what the board relies on (it fetches `/applications` with no `status` and needs the whole set). To honour the default without leaning on that wire meaning, `dashboard/page.tsx` sends an **explicit** `status=<active_states>` on the first paint (it awaits `/transitions` and `/dashboard` first, so it knows both the active set and, from `by_status`, whether any active rows exist), and the client list always sends an explicit `status` too, never the empty "send nothing" request. The guiding invariant is that the server fetches exactly the set the chips will claim, or a row-equivalent superset with no rows outside it, so the client's matching selection never renders "no matches" beside applications it merely did not fetch. Three cases follow it: active rows present (fetch the active states), an all-closed account (fetch the full non-archived row so it sees its applications rather than an empty screen), and `/transitions` failed (fetch the non-archived statuses `by_status` reports as having rows, or unfiltered only if `/dashboard` also failed).

**`archived` is excluded from the dashboard entirely: no chip, never inside "All", never fetched.** Archiving is housekeeping (§ State machine: "remove clutter without deleting history"), so an archived application has no place in the working list. `page.tsx` drops the `archived` bucket from `by_status` and every `archived` row from `facets` before they reach the client, so the chip set, the disjunctive counts, the "All" preset, and the header's tracked tally (summed from `by_status` without the archived bucket) are all non-archived by construction; the client also filters any `archived` row out of the rendered items defensively, so nothing leaks even in the `/transitions`-failed fallback. Archived applications remain reachable on the **Board's closed rail** (§ Board view), which is their home.

**The filter state lives in the URL (`v1.10.0`), so a filtered view is linkable, reload-survivable, and back-button-correct.** The four filters become query params (`status`, `company`, `source`, `japanese_level`), and three rules govern them. (1) **A bare URL is the default (active) view**: the client omits `status` when the selection equals the active default, so `/dashboard` stays clean and reload-stable; every other selection, including the "All" preset (which serialises the full non-archived set), carries an explicit `status` so it is shareable and survives reload. (2) **A pasted URL filters the first paint**: `dashboard/page.tsx` reads `searchParams` into its initial `/applications` fetch, so a shared link renders filtered from the server rather than arriving default and correcting client-side. (3) **Junk degrades to the default, never to empty and never to a `422`**: server-side that is `ListQuery`'s existing ignore-bad-input contract (§ Query layer); client-side, a `status` param whose members are all unknown intersects against the rendered chips to nothing and so falls back to the active default. The routing goes through `i18n/navigation` (`useRouter`/`usePathname`), never the `next/navigation` originals, so the locale prefix survives; and it uses `replace`, not `push`, because refining one view is not navigation and should not stack history. **One decision recorded here:** the zero-chips "show nothing" state is deliberately **not shareable**. It is a client-only transient, so it sends no `status` and reads as the default on reload, rather than growing a wire encoding for a momentary UI state.

**The stage-chip row shows the active stages plus `accepted` inline; the other closed stages have no chip (`v1.11.0`).** The seven active stages and `accepted` (the outcome worth keeping in view) render as chips whenever they have rows; `declined`, `rejected`, `ghosted` and `withdrawn` do not, since thirteen chips is past the ~10 Baymard found scannable and those graveyard stages are rarely filtered on their own. They are not unreachable: the "All" preset and a shared `?status=` URL still select them, and a closed stage reappears as a chip whenever it is the current selection, so a lit chip is never hidden; and the Board's closed rail (§ Board view) shows every closed stage with cards in full, which is their home. The split is the fetched `active_states`, so nothing here enumerates the FSM; when that set is unknown (`/transitions` failed) every stage-with-rows renders inline as before.

The **stat cards** (`v1.10.0`) ride the same payload: `response_rate`, `screening_success_rate` and `ghost_rate` (percent of *applied* applications that the company replied to, took past the resume screen, or ghosted, all read from `timeline_entries` so a later revival does not erase that a reply happened) and `avg_days_in_stage` (the mean of the board's `days_in_stage` anchor across in-flight applications).

**`screening_success_rate` is `response_rate` without the rejections, and the point is the gap between them.** A response is the company replying at all, so a form rejection counts as one. Screening success is the company saying yes to the 書類選考: `phone_screen`, `technical`, `final_round` or `offer`, the whole advanced set rather than one state, because a company that skips the phone screen has still passed the resume, and a casual 面談 is recorded as `phone_screen`, so it counts as the pass it is. The controller says exactly that in one line, `RESPONSE_STATES = ADVANCED_STATES + rejected`, so the two rates cannot drift apart. Reading them together separates the two failure modes a search actually has: a low response rate is a targeting or channel problem, nobody is reading it, while a high response rate over a low screening success rate is a resume problem, they read it and said no. Success here is a fact the timeline holds, so a rejection at the final round does not retract it, which is the same reading `response_rate` already takes and the reason neither is computed from the current status (§ State machine records what that inference used to cost). Each is `null` until there is data, and the card hides rather than showing a misleading `0%`. They are cards beside the existing `avg_days_to_offer` line, not a dedicated `/insights` route: a new page and nav weight for one user is not worth it, and if the cards ever earn promotion the queries move with them. `DashboardController` owns all of it; the cache key (`count + MAX(updated_at) + Date.current`) already changes exactly when any of these could.

**`avg_days_to_offer` scopes the user *inside* its derived table, and that is not optional.** It reads the moment an offer was recorded from `timeline_entries` rather than from `updated_at`, which drifts on any later edit, and it does so with a `DISTINCT ON` subquery to take the *first* offer per application. Postgres cannot push an outer `user_id` filter through `DISTINCT ON`, so a subquery without its own predicate computes the first offer for every application **in the database** and then discards all but one user's: work that grows with total rows on an endpoint that is per-user by definition. The join therefore carries `applications.user_id = :user_id` itself. A partial index (`index_timeline_entries_on_offer`, on `(application_id, created_at) WHERE to_status = 'offer'`) supplies both the filter and the sort the `DISTINCT ON` needs; without it this was a sequential scan and sort of the whole table. Neither is a micro-optimisation deferred until scale: the memoized stats cache above keys on `MAX(updated_at)`, so **any** edit to **any** application sends the next dashboard through this query, and `Date.current` guarantees at least one pass per user per day regardless.

**Both rates take their denominator from `applied_at`, not from the current status.** "Applications that were actually applied to" is a fact about the past, and status is a pointer that keeps moving: the original `where.not(status: %w[wishlist draft])` was only correct until a pre-application row left those two stages, which archiving it does. A `wishlist` row the user archived reads as neither `wishlist` nor `draft`, so it entered the denominator as though it had been applied to, while the numerator (a `timeline_entries` read) could never match it, because nothing ever happened to it. Every such row dragged both rates down, and archiving is housekeeping the user is *encouraged* to do. `applied_at` is the right anchor because it is written exactly when an application becomes one, on creation into `applied` and on any transition into it, and never cleared; `avg_days_to_offer` in the same controller already gated on it. It also cannot rot the way a status list does: the FSM can grow another exit from `wishlist` tomorrow and this denominator stays correct without anyone remembering it exists. The § State machine change above (withdrawal now starts at `applied`) closes the same gap from the other end, and neither fix depends on the other.

The **stage chips** are a third filter type beside these two dropdowns, and the boundary between them is load-bearing. Chips OR within themselves and AND against company and board, so the presets above them ("All", "Active", "None") rewrite only the chip selection: clicking "All" restores every stage and **keeps** the chosen company. "All" means every *non-archived* stage (archived has no chip, above); "Active" and the default view are the same `active_states` set, and "Clear filters" resets the stages to that default rather than to "All". Resetting all three filter types is what "Clear filters" is for; a control inside the stage group that also cleared a dropdown would drop a filter the user never asked to lose. One chip renders per non-archived status that *has* rows (`by_status` is `group(:status).count`), so the row is however many stages the user has actually used, not all thirteen; and it is sorted against `states` from the transition table, because `GROUP BY` without `ORDER BY` returns plan order and the chips would otherwise reshuffle between reloads.

**No filter control is `disabled` while a page is in flight** (`v1.11.1` generalised the chip `<fieldset>`'s rule to the search box and the three dropdowns). A disabled element cannot hold focus, so the browser blurs it to `<body>` the moment a fetch starts: a keyboard or screen-reader user pays a full tab back for every consecutive search, and the search box (which submits on Enter, so it disables itself on every use) was unusable without a mouse. **Block the handler, not the control**: `applyFilters` early-returns on `loading`, so an interaction mid-fetch is a no-op with focus intact. The two buttons that stay `disabled` are not filters: "Load more" is a busy indicator that swaps its own label, and "Clear filters" unmounts when the filters clear.

**The search box's Enter is gated on `isComposing`** (`v1.11.1`). Enter is how an IME confirms a kanji candidate, and without the guard that keystroke leaks through as a form submit and searches half-composed text, which is the primary way this box gets typed in on a Japanese-market app. The guard reads `e.nativeEvent.isComposing` on keydown, because React does not surface `isComposing` on its synthetic event, and preventing the default there is what stops the implicit submit.
...
The CSV is a **convenience view**: a spreadsheet, one row per application, no blobs and no timeline. It recovers a table, not an account. Its columns are a **hand-curated allow-list** (`Exports::ApplicationsCsv::COLUMNS`), not every column: the Japan-market and visa layers (`channel`, `japanese_level`, `sponsorship`, `status_of_residence`, `hiring_entity`, the comp structure) are deliberately absent, because the CSV is the *legible* view and a dozen market columns turn a readable spreadsheet into a database dump. Completeness is the archive's job, not this one's: `account.json` carries every column, so nothing is lost, only relocated to the artefact built to hold everything. Adding a market column here is a decision to make the convenience view less convenient, taken column by column, not a default.

The archive is the **data-safety artefact**, and the reason this exists at all: the real job-search history lives in one self-hosted Postgres (§ Deployment), with no managed backups behind it. Scheduled `pg_dump`s cover that from the outside (§ Deployment); this covers it from the inside, and is the leg the user can pull without a provider, a cron runner, or a shell. It contains `account.json` (user, every application with every column, every timeline entry) plus the PDFs under `resumes/` and `cover-letters/`, named by `Application#download_basename` (§ Download filenames), the same method the per-application download endpoints use, so a file means the same thing whichever door it left by. `account.json` carries a `schema_version` so a future importer can tell what it is reading, and each application row names its own files, so the mapping survives even when a segment is unhelpful: the id in the name is what makes it unique, the company and role are only there to be readable.

**"Every column" means every column, so `AccountArchive` merges `posting_snapshot` back in the way `#show` does.** It is not free to say: `Application#as_json` drops the snapshot beside the two blobs, and this file rode `as_json` alone, so the archive shipped without the one column no other export carries. The CSV excludes it by design (above), and the snapshot exists precisely because postings get taken down, so its absence here was the archive failing at the single thing it is for. The rule this leaves behind is worth more than the line of code: **a serializer written for the read paths is the wrong default for an export**, and any column `as_json` learns to omit for weight or leanness has to be re-added here in the same change.
...
**Workers run inside Puma.** `config/puma.rb` has `plugin :solid_queue if ENV["SOLID_QUEUE_IN_PUMA"]`; that variable must be set on the `api` container (§ Deployment). There is no separate worker service.
...
The two hourly tasks carry no timezone, and since `solid_queue` 1.5.0 a zone-less schedule resolves in `config.time_zone` (`Tokyo`) rather than the container's system zone (UTC on Railway). For an hourly-at-minute-N schedule those are the same set of instants, because JST is a whole-hour offset, so the change is a no-op here. A **daily** zone-less task would not get that reprieve: give any new daily task an explicit zone, as the two above have.
...
**The shared demo account is excluded from the scope**, and the reason is the claim rather than politeness. Its seed carries a deliberately overdue follow-up so the Upcoming agenda is never empty for a visitor (§ `Demo::ResetService`), and `Demo::ResetService` destroys the account hourly, taking the `TimelineEntry` that claims the reminder with it. The exactly-once anchor below is therefore erased every hour, so that one seeded row would earn a fresh digest every morning, addressed to a mailbox that exists to be logged into rather than read. `InterviewReminderJob` needs no equivalent: it only reaches users with a push subscription, and the same hourly reset deletes the demo's.
...
**Production uses Resend's alternate STARTTLS port `2587`, not the standard 587/465.** This dates from Railway, which blocked outbound SMTP on those ports; the home connection has no such block, but there is no reason to change a working config, so the port stays. The `From:` domain must be verified in Resend first.
...
- **Rack::Attack**: throttle counters go through `Rails.cache` (Solid Cache), so they are shared across Puma workers rather than counted per worker.
  - **`Rack::Request.forwarded_priority` is pinned to `[:x_forwarded]`, and every per-IP throttle depends on it.** `Rack::Attack::Request` subclasses `Rack::Request` and overrides neither `#ip` nor reads `env["action_dispatch.remote_ip"]`, so `req.ip` is governed by *Rack's* rules and not by `config.action_dispatch.trusted_proxies` (which this app has never set). Rack 3.2 consults the RFC-7239 `Forwarded:` header **ahead of** `X-Forwarded-For`, and Railway normalises the latter while leaving the former alone, so a client could simply declare the IP it wished to be throttled as and rotate it per request. That defeated `sign_in`, `prefill`'s per-IP leg, and `auth/passkey`, which is the *only* limit on an unauthenticated endpoint that writes a cache row and does WebAuthn crypto per call. The pin is a global Rack setting, so ActionDispatch's `request.ip` inherits it. Same lesson as `normalized_path` below, one layer down: a security control must not key off a value the client can choose.
  ...
  - `sign_in`: per-IP, plus **email-keyed** throttles (`10/5min`, `50/hour`) capping guesses against a single account across all IPs. IP-only throttling is defeated by a botnet or a shared NAT egress. The sign-up endpoint `sign_in` used to sit beside is gone (§ Registration is closed), and with it the spam-account and outbound-mail vector that its 3/hour cap existed to close; the unauthenticated writes left are `sign_in` and the two passkey ceremony paths, whose per-IP throttle (and why it has no email-keyed backstop) is in § Passkeys.
    - **The email discriminator reads the body in whatever encoding it arrived in, not just JSON.** `sign_in_email` originally recovered the target address through `JSON.parse` alone, so a form-encoded, multipart or query-string sign-in (Devise authenticates all three exactly as readily) raised, hit the rescue, and returned `nil`, and a `nil` key means no counter and no limit. That fails open on the **only** account-level brute-force defence in this app: Devise `:lockable` is deliberately not enabled, so these two throttles are the whole of it, and the per-IP leg still firing left an attacker with N addresses `5N` guesses a minute against one named account with no account-level ceiling at all. The published demo credentials made the target concrete. It now tries JSON first (read from `rack.input` and rewound, since Rack's form parsing does not cover it) and falls back to `req.params`, which covers form-encoded and multipart bodies *and* the query string. One counter serves all encodings, so alternating between them does not buy a fresh budget. **Third instance of the same lesson as the two rules above: a guard must key off what the request means, never off something the client chooses**, and the encoding is exactly as client-chosen as the header and the path suffix were.
  ...
  - `applications/write`: **per-account** caps (30/min, 300/hour) on `POST /applications` and `PATCH|PUT /applications/:id`, the two requests that carry a blob. Per-account for the same reason as exports. The throttle covers every write to those paths rather than only the ones with a file attached, because deciding *inside Rack middleware* whether a multipart body contains a PDF means parsing the body Rails has not parsed yet, to save a counter increment on a request that is cheap either way. `DELETE` is deliberately not throttled: it is the one write that gives storage back. `POST /applications/prefill` and `.../transition` do not match these patterns: prefill has its own caps above, and the path regex is anchored on `/\d+\z`, which is why the two endpoints below need their own.
  - `applications/transition`: **per-account** caps (30/min, 300/hour) on `PATCH /applications/:id/transition`. It matched *nothing* until 2026-07-28: it fails the `/\d+\z` anchor above and no other guard claimed it, so the discriminator returned `nil` and the guard failed open, which is precisely the fail-open `normalized_path` exists to prevent. Every call writes a `timeline_entries` row and the FSM permits an unbounded `applied → rejected → applied` cycle, so it was a write loop with no ceiling. `TimelineEntry::NOTE_MAX_LENGTH` (2,000) is the other half, for the reason the application ceiling below states in general.
  - `ai/talking_points`: **per-account** caps (5/min, 30/hour, 60/day) on `POST /applications/:id/talking_points`. It had no throttle of any kind while `prefill`, which is cheaper, had four, and it is the most expensive call in the app: it base64-encodes the stored resume (up to 1 MB) inline into a paid Claude request inside a Puma request thread. Per-account only, with no per-IP leg, because the endpoint requires a decodable JWT and so has no unauthenticated surface for a coarse cap to protect. The **published demo credentials** are what made this sharp: not "a user can overspend their own budget" but "anyone can".
...
- **Param filtering**: lograge logs `request.filtered_parameters` **in full** on every production request, so `filter_parameter_logging.rb`'s list is the whole defence for anything carried in a request *body*. (The `Authorization` header and cookies are never logged at all.) It filters the obvious `passw`/`email`/`token`/`secret` family, and, since an audit found them absent: the **WebAuthn ceremony payloads** (`credential`, `challenge`, `rawId`, `signature`, `clientDataJSON`, `authenticatorData`, `userHandle`, the last being `users.webauthn_id`, which `User#as_json` deliberately strips from every API response), the **push subscription's `endpoint`/`p256dh`/`auth`** (together, the addressing *and* keying material for a browser's push channel; note `:_key` matches `public_key` but not `p256dh`, and nothing matched `auth`), and the user's **free text** (`text`, `notes`, `note`, `posting_snapshot`), on log-volume grounds as much as privacy: a pasted posting can be 2 MB.
...
How the running app is watched. Three things, all of them cheap enough to leave on:

- **Structured JSON logging** via `lograge` in production: one line per request with `request_id`, controller, action, status, duration. It also emits `request.filtered_parameters` in full, which makes `config/initializers/filter_parameter_logging.rb` the only thing standing between a request body and the log. That list therefore names the WebAuthn ceremony payloads, the push subscription's `endpoint`/`p256dh`/`auth`, and the user's free text, as well as the obvious `passw`/`email`/`token` family. The `Authorization` header and cookies are never logged at all.
- **Error tracking** via Honeybadger in production; API key from an env var, never hardcoded.
- **`/up`** pings Postgres and returns `200` / `503`, so the `api` container's Docker `HEALTHCHECK` (§ Deployment) fails fast on dependency loss. The Rails 8 default only checks that the app booted. It no longer pings Redis: there is no Redis.
...
The brand book lives in the Claude Design project named in `BRAND.md`, not in this repo; `globals.css` is the only place those tokens enter the app, through Tailwind v4's `@theme inline`. Nothing imports the brand book at build time, so `globals.css` is a hand-kept mirror and the values here are the ones that actually ship. Twelve colours (the nine brand hues, plus three that exist because the obvious choice failed a contrast requirement: `--color-danger`, a warm madder (`#96291D`) for destructive actions, error text, and terminal-negative statuses, always applied through opacity modifiers (`text-danger`, `bg-danger/10`, `ring-danger/30`) and never stock Tailwind `red-*`; `--color-saffron-ink` (`#7A4D10`), the AA-contrast text cut of saffron, since bare `text-saffron` (`#E8A04A`) is only ~2:1 on linen: saffron carries meaning as a **fill or ring**, and any saffron *text* (the offer badge, the follow-up and agenda dates) uses `saffron-ink` for ~6.5:1; and `--color-rule-strong` (`#847D6B`), which carries the borders of interactive controls, `dune` being 1.50:1 on linen and 1.36:1 on sand where WCAG 1.4.11 requires 3.0, leaving `dune` for decorative dividers), **one typeface** (Plus Jakarta Sans, display through caption) with labels on the system mono stack, and **radius `0`**: the sharp corners are the editorial voice, not an oversight.

As of brand book v1.2 the two sides agree on all twelve colours *and* on type. That second half is new: the app shipped Fraunces, Manrope and IBM Plex Mono until the T1 swap below, and `BRAND.md` is where the direction was decided and argued.

**Type is T1: one variable family, no serif, no second webfont.** Plus Jakarta Sans loads through `next/font/google` in `web/app/[locale]/layout.tsx` and again in `global-not-found.tsx`, in the same variable form both times so the emitted files are content-hash-shared rather than duplicated. It is a **variable build** on the `wght` axis (200..800), which is the whole reason one file can carry five weights: the wordmark's 800 and 300, h1 700, display and h2 600, body 400. Enumerating `weight: [...]` would fetch a static instance per weight and defeat that, so don't. Labels and metadata use `ui-monospace, SFMono-Regular, 'SF Mono', Menlo, Consolas, monospace`: a system stack, so there is no font file behind `.kk-label` or `.kk-num` at all. That is **one `@font-face` declaration** where the previous three families needed five (Manrope, Fraunces normal, Fraunces italic, Plex Mono 400, Plex Mono 500), and fifteen before those went variable.

One thing about `subsets` that reads wrong at a glance: a clean build emits **four** `woff2` files for the one family, not one. `subsets: ["latin"]` chooses what gets **preloaded**, not what gets emitted; Next writes every `unicode-range` slice Google's stylesheet declares. Only the latin slice (27 KB) carries the `.p.` preload marker and is actually fetched, because the other three (latin-ext, vietnamese, cyrillic-ext) cover codepoints no page in an English/Japanese app contains. So the transfer is one file; the build output is four, and a count of files on disk is not a count of bytes over the wire.

The typeface changed because the old one failed at text sizes, not because the brand got bored. Fraunces at `opsz 144` weight 300 italic is drawn for 72px and up; the dashboard and board card titles rendered it at 20px, where the joins go hairline and the italic `a` and `e` close up. That is a legibility defect in the two densest screens in the app. The trade taken in exchange is stated plainly in `BRAND.md`: the wordmark loses its italic, and the brand reads product-first rather than magazine-ish.
...
- **Display is a class, not an element.** `.kk-display` (weight 600, `-0.04em`) is the homepage hero and nothing else; an `<h1>` anywhere takes the h1 role (700, `-0.03em`) instead. Keeping the two separate is why both roles exist: a page can carry one display-scale line and still have a normal heading hierarchy under it. The scale then has **two** heading steps rather than three, so `h3` shares `h2`'s cut (600, `-0.018em`); a step nothing specifies is a step that drifts. Weight and tracking travel together per role, so read both from the same row of the table in `BRAND.md`, never one from one row and one from another.
- **The weight ladder is not monotonic, and that is deliberate.** Display at 600 is *lighter* than h1 at 700, and equal to h2. Weight reads against size: display only ever renders at `2.75rem` and up, where a heavier cut sets the line solid, while h1 renders small enough that 700 is what separates it from h2. Anyone "correcting" display back above h1 is reading the number without the size beside it. The wordmark keeps **800**, the heaviest thing in the app, because a mark is drawn once at a fixed size, so the argument does not reach it; upstream it has its own `--kk-fw-wordmark` token for exactly that reason.
- **Weight comes from `font-weight`, not `font-variation-settings`.** The previous type needed the latter, because Fraunces' `opsz` axis has no CSS property of its own. Jakarta varies on `wght` alone, which `font-weight` drives directly on a variable build, so the variation-settings rules are gone. The heading rules are still **unlayered**, which means they outrank Tailwind's layered `font-*` utilities: a `font-semibold` on an `<h1>` does nothing. That was already true before the swap and is why no heading in the codebase carries one.
- **Negative tracking is a Latin display device**, so a `:lang(ja)` rule resets it to `0` on plain headings (kanji and kana are drawn to full-width boxes). `.kk-label` and `.kk-display` are excluded from that rule because each carries **its own** `:lang(ja)` rule, not because either is exempt. That distinction is the whole content of the exclusion, and getting it wrong is what let `.kk-display` keep `-0.04em` under Japanese: it was excluded on the strength of "sets its own tracking", which was only ever true of its *Latin* value, so the homepage hero was the one heading in the app still running a Latin display device over kana and kanji, at 44px and 72px, and T1 tightened it from `-0.035em`. Its ja rule now zeroes the tracking and opens the leading to `1.25`: the Latin `1.02` assumes an em box mostly empty above the x-height, which CJK fills, and the ja hero wraps to three or four lines at 375px. **Anything added to that `:not()` list owes a `:lang(ja)` rule of its own.**
...
`.kk-wordmark` (upright "karir" at 800 + upright cobalt "kalyan" at 300, weight and colour carrying the two-word structure where the italic used to), `.kk-label` (mono eyebrow), and `.kk-num` (mono ordinal, tabular figures) are the only other custom classes; everything else is Tailwind utilities.
...
3. `DELETE /api/auth/session` hits Rails to rotate the JTI, then clears the cookie. The button that calls it lands the visitor on **`/`, the marketing page, never `/sign-in`**: a deliberate sign-out is leaving, and dropping someone on the sign-in form says "you were kicked out, sign back in", which is the expired-session bounce's message (below) rather than this one's. The route guard makes this safe in either order: `/` is a `PUBLIC_PATH`, so the now-tokenless visitor is not bounced back to the dashboard.
...
- where it lives: `bytea` in a single self-hosted Postgres (§ Deployment), with a nightly `pg_dump` run by a GitHub Actions self-hosted runner (§ Backups) and uploaded as a workflow artifact to the private `karirkalyan-backups` repository, never git-committed, expiring after 60 days;
- who else touches it: **six** parties, and the pages name all six, because a recipient you decline to name is the one the policy exists to disclose:
  - **Cloudflare**: the tunnel that fronts both hostnames terminates TLS at Cloudflare's edge, so every request, including sign-in credentials and any resume upload in flight, is decrypted there before `cloudflared` re-encrypts it to the origin. That makes Cloudflare a sub-processor of *every* request, not just the backup, which is why it leads the list rather than trailing it;
  - **GitHub**: the nightly `pg_dump` is uploaded there as a workflow artifact, which means **GitHub holds a copy of every resume**. It is easy to forget because the backup repository is private and the workflow is boring, and it is precisely the kind of omission that makes a policy false;
  ...
  - **the browser's push service** (Google's, for Chrome, which the Android-first install story makes the common case): reached only if the user enables notifications, and the **one recipient on this list that cannot read what it carries**. `Push::Notifier` POSTs to the endpoint the browser handed us, and the payload is encrypted to keys only that browser holds, so what crosses is ciphertext. It is on the list anyway, because it still learns that a given endpoint was sent something and when, and because "that is the entire list" is the sentence the page lives or dies on. It was missing from this list, and from the pages, from `v1.6.0` until `2026-08-05`;
...
- one outbound request that is *not* a sub-processor: AI pre-fill makes the **server** fetch the job posting, so the site hosting it (LinkedIn, a company careers page) sees a request from the server's own IP, not from the user's browser. No personal data goes with it, so it does not join the list of six; the pages say it anyway, because it is the only case of the app talking to a host the *user* chose, and because the answer is a good one: the job board never learns who looked;
- what is *not* there: no analytics, no tracking pixels, no advertising, no third-party JavaScript. **Three** cookies, all functional: the `session` cookie that holds the JWT, next-intl's `NEXT_LOCALE`, which remembers the chosen language, and `account_email`, which lets the header render the initials chip without a second request (`v1.7.0`, `#76`). None is a tracker, and the pages say "three functional cookies", not "no cookies", see below. This list said **two** from `#76` until `2026-08-05`, while the pages themselves said three: the pages were right;
- **one `localStorage` key**, `kk.pins.v1` (§ Pinned applications), the only client-side storage in the app that is not a cookie. It never leaves the device, so it is not "collected" in any sense this page uses, and functional storage needs no consent banner either way. The pages name it regardless, because they enumerate client-side state exhaustively and an inventory with an unnamed entry in it is the kind of claim a reader can falsify with devtools;
...
The sections, in order, and what each is doing there:

1. **Upcoming agenda.** The shortest block on the page and the only one about *today*, so it leads.
2. **The applications list.** The working list, and the reason the page exists.
3. **Stat cards and the `avg_days_to_offer` line.** Ambient numbers. Nothing to act on, so nothing is lost by reading them after the list. Four cards, two per row on a phone and one row from `sm` up; the response rate and the screening success rate are adjacent on purpose (§ The dashboard payload).
4. **Ghost-risk card.** Below the list as of 2026-08-19, having led the page since it shipped.
5. **Your data.** Account admin: an email address, a join date, and the two export links.

**Ghost risk moved below the list, and the argument that kept it on top is what makes moving it safe.** It is still the only block that asks the user to *do* something, which is why it led; but every row it names is already flagged inside the list itself (`page.tsx` passes `atRiskIds` down), so the nudge does not depend on the card's position. What the card adds is the one-click `ghosted` transition, and that is a decision to take after reading the pipeline rather than before it. It renders nothing when nothing is at risk, so on a healthy account the position costs nothing at all. The cost is honest and worth writing down: on an account that *does* have at-risk rows, the prompt to clear them is now a scroll away.

**Your data is last because it is not part of the day's work.** Sitting between the agenda and the list, it pushed the list below the fold to show two facts that never change and two links that are visited deliberately. `/privacy` promises the export surface exists, not that it is the third thing on the dashboard.

**The Upcoming agenda shows the next seven days, plus anything overdue** (`AGENDA_WINDOW_DAYS`, in `web/app/lib/agenda.ts`). The API sends every dated commitment it has, and the three-row cap was the only thing standing between that and the dashboard: the wrong filter on its own, because an interview eleven weeks out is not something to do this week and it was taking a row from the follow-up that is. The window decides which items are *eligible*, the cap decides how many of those fit. Three boundaries, each deliberate:

- **No lower bound.** Anything in the past is inside the window however old, because an overdue follow-up is the most actionable row on the page.
- **`residence` is exempt.** A Certificate of Eligibility takes 63 days (`Visa::COE_LEAD_TIME_DAYS`), so a residence clock that first appears a week before expiry appears too late to act on; lead time is the entire reason the API windows that row at 90 days in the first place. It is at most one row, and it is already windowed once on the server.
- **Nothing is dropped.** Whatever falls outside the window folds behind the same "Show more" toggle as the rows past the cap, so every item the API sent is one click away.

The section renders nothing when the window is empty, even when later items exist. That is the promise it always made (a dashboard that is quiet when there is nothing to do), narrowed from "nothing ahead" to "nothing ahead this week": a distant interview is still on its application, on the board, and back here as it comes within the week.

**Both action cards cap their visible rows at three**, behind a "Show more" toggle, and each re-ranks its own three for the read the section is for. `GhostRiskCard` sorts by fewest `business_days_in_stage` first (the most recently touched of the at-risk set) rather than by the server's longest-silence-first. `UpcomingAgenda` sorts by absolute distance from now, so a week-overdue follow-up outranks an interview three months out, rather than by the server's chronological order. Both orderings are unchanged on the wire (§ The dashboard payload); only the local render order and the visible count differ.
...
A move applies optimistically via `useOptimistic` and calls the existing `transitionStatus` server action. A success now speaks a toast ("Moved to {label}"); on failure the card snaps back to its source column and a toast shows the resolved error (§ Toast feedback, § Server-side error messages), which since `v1.11.0` is the shared toast primitive rather than the board's own `role="alert"` notice. A `409` / `stale_record` additionally triggers `router.refresh()`, since the board's copy of that application is stale by definition. `revalidateApplication()` in `actions.ts` revalidates `/board` alongside `/applications/[id]` and `/dashboard`, so moves made elsewhere reach the board on next render.
...
The dashboard opens on the active stages, which on a busy search is still more rows than the two or three the user is actually waiting on. Pinning hoists those to the top, so the daily-driver view leads with the applications that matter this week rather than with whatever the cursor order put first.

**Pins are device-local on purpose, and that is a feature, not a shortcut.** They live in `localStorage` under `kk.pins.v1`, a JSON array of application ids. Nothing about a pin is worth a column: it is a transient attention marker, not a fact about the application, and it changes several times a week while the row it points at does not. Storing it server-side would mean a migration, an endpoint, a cache-key interaction with the dashboard's memoized stats, and an export question, all to sync a preference between the one user's phone and his laptop that is *better* unsynced anyway (what he is watching on his phone at lunch is not what he is watching at his desk). The cost is stated plainly rather than hidden: **pins do not survive clearing site data, and they do not follow the user to another device.** They are also not in the account export, for the same reason they are not in the database.

**`app/lib/pins.ts` holds the pure logic and is unit-tested; `app/lib/use-pins.ts` is the React seam.** The split is the one `excerpt.ts` already set: parsing, the cap, the toggle and the hoist are pure functions with no DOM in them, so they are testable through the Vitest seam without a browser, and the hook is the thin part that touches storage.

**Anything unreadable in that key reads as no pins.** `parsePins` accepts only a JSON array, keeps only positive safe integers, dedupes, and truncates to `MAX_PINS`; every other input (absent, malformed JSON, an object, a string of ids, a hand-edited array of forty) returns `[]`. `localStorage` is user-writable and survives every deploy, so a shape this code stops recognising is not a hypothetical, and the failure has to be "your pins are gone", never a crashed dashboard.

**A pin reorders; it never selects.** `sortPinnedFirst` hoists pinned rows to the front of the *already-rendered* list, in pin order, leaving every other row where it was. Two consequences follow, and both are deliberate:

- **A pin cannot defeat a filter.** If the user filters to `offer` and a pinned application is `applied`, that pin does not appear. The alternative (fetching pins by id and prepending them) would put a row on screen that contradicts the chips directly above it, which is the same lie the archived-row exclusion exists to prevent.
- **A pin cannot see a page that was not fetched.** The list is cursor-paginated at 10; a pinned application on page four hoists when "Load more" reaches it, not before.

Because both cases are silent by construction, the list says so: a small note above the rendered rows reports how many pins the current view is not showing. Without it a pin that is merely off-page is indistinguishable from a pin that was lost, which is the reading that makes a user stop trusting the feature. It rides with the list and not with the two empty states, which already explain themselves ("no applications match these filters", "every stage is hidden").

**There is a third way a pin goes missing, and it is the one that needed an escape hatch: the application was deleted.** Then no filter change and no amount of "Load more" can ever bring the row back, so the per-row toggle, which needs a row to hang on, cannot reach that pin again. The note therefore carries an **Unpin all** control, and its wording no longer prescribes two remedies that a deleted pin makes impossible; it names all three causes instead. Nothing prunes the set automatically: doing that safely needs proof the whole unfiltered list is loaded, which cursor pagination cannot cheaply supply, and guessing wrong silently discards a pin the user placed by hand. Clearing is all-or-nothing because the note cannot say *which* pins are missing in a form the user could act on, and with a cap of three, re-pinning costs almost nothing. It `removeItem`s the key rather than storing `[]`, so a cleared set reads exactly like a browser that never had one.

The hoist is the one place the list deliberately re-orders across pages. `sortByImportance` is applied only to a first page precisely because re-sorting accumulated results interleaves fresh rows into the middle of already-seen ones; hoisting is exempt because it moves a **named set of at most three** to a **fixed position**, so nothing shifts unpredictably under the reader, and the ordering it produces is the one the user asked for by hand.

**`usePins` is a `useSyncExternalStore` subscription, not state seeded in an effect.** `localStorage` *is* an external store, and the primitive built for one gives three things at once: `getServerSnapshot` (a stable empty array) is what the server render and the hydration pass read, so there is no markup for the client to contradict; the subscription covers a write in this tab *and* a `storage` event from another, which fires only in the other tab and so cannot cover both on its own; and there is no render-then-immediately-re-render, which is what the effect version costs and what React's own lint rule flags. The one rule it imposes is that `getSnapshot` must be referentially stable while the data is unchanged, or React re-renders forever, so the parse is cached against the raw string it came from.

**Pins therefore appear a beat after first paint, and that is the honest answer rather than a limitation.** Every route renders on the server (the CSP nonce requires it), and the server cannot know what is in this device's `localStorage`, so any markup it emitted about pins would be a guess the client had to correct. For a reordering of at most three rows, that is not worth a cookie to avoid.

**Two failures get a toast, because in both the button appears to do nothing.** Pinning and unpinning need no confirmation (the row moves), but a **fourth** pin is refused, and a `setItem` that **throws** (private-mode Safari, a full quota) leaves nothing to read back. Since the rendered state is read *from* storage, swallowing that second case yields a control that neither acts nor speaks, which is the failure the cap's own message exists to prevent, so `toggle` returns `"unavailable"` and the list says so.

**The cap refuses rather than evicts.** Silently dropping the oldest pin would be the friendlier-looking choice and the wrong one: three is small enough that the user knows what is pinned, so an unrequested eviction reads as the app losing one.

**Every row carries the control, and the pinned state is shape, not colour.** The toggle is a real `<button aria-pressed>` beside the row's link (never nested inside it, which is invalid HTML), rendered on every row rather than revealed on hover, because this is an Android-first PWA and hover does not exist on the device it is built for. Pinned draws a filled pin, unpinned an outline, so the state survives WCAG 1.4.1 the same way the filter chips' real checkboxes do; the saffron-ink tint and the saffron left rule are redundant scan aids on top. **Its accessible name carries the company** ("Pin Mercari to the top"), while its tooltip stays short: the button is the one thing in the row that sits outside the link naming the application, so ten identically-labelled buttons would be a list a screen-reader user cannot navigate.

**Scope: the dashboard list only.** The Board (§ Board view) is a different question ("what is in each stage"), its cards are already grouped by the thing that orders them, and a second pin affordance there would need its own answer for what "top" means in a column. If pins earn a place there later, they read the same `app/lib/pins.ts`.

### Toast feedback

**One toast primitive for every write (`v1.11.0`).** `web/app/components/toast.tsx` is a context (`ToastProvider`, mounted once in the `(app)` layout) plus a single `aria-live="polite"` region and a `useToast()` hook exposing `success` / `error`. It unifies what were three inconsistent surfaces: the board's hand-rolled `role="alert"` notice (now gone), the silent successes on the transition surfaces, and the void a reverted optimistic move used to fall into. A transition or board move confirms with a toast ("Moved to {label}"), and the board's optimistic-revert path speaks its reason through the same region every other write reports to.

It is **polite, not assertive**: a toast is a result of the user's own action, so it waits rather than cutting in, the same rule the list's `aria-live` count region already follows. Toasts auto-dismiss (5 s) and carry no entrance animation, so there is nothing for `prefers-reduced-motion` to fight, and the stack sits above the phone tab bar. It confirms the writes a user waits on: the transition and board moves, a detail-page **save** ("Saved") and **delete** ("Application deleted"). A delete navigates away before it could toast in place, so its confirmation rides a one-shot `?toast=deleted` query that the dashboard's `ToastFromParam` reads once and strips (via `replace`, no history). Deliberately scoped even so: the detail page's inline **field errors** stay inline, because a form-field error belongs beside its field, not in a corner toast.
...
Three deliberate exceptions, all importing from `next/navigation` on purpose:
...
- `useSearchParams` in `app/components/toast-from-param.tsx`. It is a **read** hook, not navigation: `i18n/navigation.ts` re-exports nothing to replace it, and there is no prefix to preserve when you are only inspecting the current query string. Listed here because the file carries the same warning the one above does, and a count of two was itself an invitation to "fix" it.
...
**`web/scripts/check-fsm-copies.mjs` (`v1.11.0`) proves the "the board never mirrors the transition table" claim rather than asserting it.** The README line had been written three times and been wrong twice, each time because the grep behind it was never run to the end. The script reads the canonical state names from `types.ts`'s `Status` union (the one place allowed to hold them) and scans every other `web/app` `.ts`/`.tsx` for a cluster of three or more distinct state-name **string literals** within a few lines of each other, i.e. an array or `Set` literal. It runs as `npm run lint:fsm` in the same `verify` job, beside the catalog parity check.

The allow-list is **justified line by line in the code it guards**, not in a list over here: a legitimate hardcoded set carries an inline `fsm-allow: <reason>` marker on or just above its declaration, and the marker is the justification, read in review. The five that carry one are all pure affordance or illustration, none of which a fetched fact could replace: `COLUMN_ORDER` (board display order), `CONFIRM_REQUIRED` and `STAGE_NOTE_STATES` (which moves prompt, which offer a note), and the homepage `pipeline-diagram.tsx`'s `HAPPY_PATH` / `REVIVABLE` (a drawing nothing reads). A `Record<Status, …>` map like `STATUS_PRIORITY` is exempt without a marker, because it uses bare, typed keys that TypeScript already forces complete, so it cannot drift silently the way a string-literal array can.
...
`formatDate()` pins `timeZone: "Asia/Tokyo"`. The API serialises in app time, and a date-only field like `follow_up_at` parses as UTC midnight, so without the pin a viewer west of UTC would see the previous day.

**`isOverdue()` pins the same zone, and must** (`v1.11.1`). It compares date strings, so it needs a "today" to compare against, and it derived one from the ambient clock until `v1.11.1`. That made it a **hydration bug, not a display quirk**: the `web` container runs UTC on Railway (no `TZ` is set) while the browser runs JST, so between 00:00 and 09:00 JST the server and the client disagreed about the date and rendered a genuinely-overdue follow-up two different ways, a different i18n key and a different className, on the dashboard list, the detail page, and the Upcoming agenda. Today is now `Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Tokyo" })`, the same zone `formatDate` shows, and `app/lib/format.test.ts` fakes an instant inside that window to keep it there. `todayISO()` in `new-application-form.tsx` is a deliberate near-twin and stays on the local calendar: it seeds a date input the user is about to edit, which is not a comparison against a server value.
...
| `api` | built from `api/Dockerfile` | Puma, with the Solid Queue plugin (no worker service, same as before) |
| `web` | built from `web/Dockerfile` (new; Railway built this tree with Railpack and never read a Dockerfile) | `next start`, `output: "standalone"` in `next.config.ts` |
| `cloudflared` | `cloudflare/cloudflared` | The only ingress point; holds the tunnel credentials and the ingress rules |

**Both app images are non-root, multi-stage, and self-check.** `api/Dockerfile` and `web/Dockerfile` each build in one stage and run in a second, slimmer one under a uid-1000 user (`rails`; `web/Dockerfile` reuses the `node` user the base image already ships rather than creating a second one at the same uid, which collides). Each carries a `HEALTHCHECK`: `api`'s hits `/up`, the same deep check (pings Postgres, not just process boot) Railway's own healthcheck used; `web` has no dedicated health route (adding one for orchestration alone would be new app surface under the `v1` freeze, `TODO.md` § The rule), so its check is a `200` on `/`, which still proves the Next.js process, next-intl's locale routing, and `proxy.ts`'s route guard are all actually working. `docker-compose.prod.yml` wires both into `depends_on: condition: service_healthy`, so `api` never starts racing an unready Postgres and `web` never starts fronting an `api` that can't reach its own database, an ordering problem host-port `curl` loops used to paper over by polling instead of asking Docker to enforce it.

**Because those gates are hard, the health windows are sized for the slowest legitimate boot, not the typical one.** `api`'s `HEALTHCHECK` allows a **300s start period** (probed every 5s inside it, so a normal boot still flips healthy in seconds). `bin/docker-entrypoint` runs `db:prepare` to completion *before* Puma binds, so every probe fails until migrations finish, and `web` and `cloudflared` both gate on `api`. Under the original 30s window a first boot against an empty volume, or one slow migration on modest hardware, would not merely have started late: Docker would have marked `api` unhealthy, and compose would have failed the dependency and started neither of the other two, turning a delay into an outage.

**`postgres`'s healthcheck authenticates rather than pings**, for a failure that is otherwise silent. `POSTGRES_PASSWORD` is read only while initializing an *empty* data directory, so editing `DB_PASSWORD` in `.env` against an existing volume leaves the old password authoritative while `api` rebuilds its credentials from `.env` on every boot. `pg_isready` reports healthy regardless of which password works, so the stack would crash-loop on auth errors with `docker compose ps` showing `postgres` perfectly fine. The check therefore runs a real `psql` `SELECT 1` with the configured credentials, and does it over `-h postgres` rather than `127.0.0.1`: initdb's own generated `pg_hba` entry **trusts loopback**, and only the entrypoint's appended `host all all all scram-sha-256` rule verifies a password, so a loopback check would pass with any password at all. Rotating is consequently two ordered steps, `ALTER USER … PASSWORD` inside the container and then the `.env` edit (`.env.prod.example` carries the command); doing it in the other order fails the healthcheck, which is the intended outcome, since `api` is held back rather than started against credentials that cannot work.

**Only the two app containers carry a `mem_limit`, and `api`'s is 768m for a measured reason rather than a guessed one.** Both were set to 512m at the cutover, which was Railway's plan sizing carried across without being re-examined against a host that has roughly 48 GiB free. Measured against 13 hours of uptime on 2026-08-21, `api` was not sitting under a cap, it was pinned against a ceiling: `memory.current` 510 MiB of the 512 MiB limit, `anon` 493 MiB of it real Ruby heap that cannot be reclaimed, `file` and `inactive_file` both **zero**, and `memory.events`' `max` counter at 92. That combination is the whole diagnosis: the kernel had evicted every page of cache to hold the cgroup under its limit and had re-entered reclaim 92 times, so Rails was going back to disk on every miss. Nothing failed loudly, `oom_kill` was 0 and the container had 0 restarts, which is exactly why it went unnoticed from the cutover onward. `api` is now 768m, which restores headroom while keeping the limit a limit, so a leak is still contained rather than allowed to eat the host. `web` stays at 512m, using about 102 MiB of it. **`postgres` and `cloudflared` carry no limit and should not:** a hard cap on Postgres is a cap on the page cache it depends on for read performance, which is how you make a database slow without making it fail, and `cloudflared` runs at about 28 MiB.

Two operational notes attach to that. **A memory limit can be changed without recreating the container**, via `docker update --memory 768m --memory-swap 1536m <container>`, which is how this one was applied to production with no downtime; the swap figure is not optional decoration, since Compose sets `MemorySwap` to twice `mem_limit` and passing only `--memory` leaves the live container disagreeing with what the next `bin/deploy` will build. `docker-compose.prod.yml` stays the source of truth either way. And **`memory.events` is cumulative and never resets**, so its `max` value answers "has this ever happened" rather than "is this happening": the diagnostic is whether the counter climbs from a noted baseline, which was **98** at the time of the change, not the number itself.

Environment variables, unchanged from Railway in value, but split by which container needs them: `api` reads the root `.env` (gitignored; `.env.prod.example` is the committed template) for `DB_USERNAME`/`DB_PASSWORD`/`DB_NAME` (the same three that configure the `postgres` service itself, so the two halves cannot drift; `DB_HOST` is set to the `postgres` service name by compose, being the one value that is a property of the topology rather than of the credentials), `DEVISE_JWT_SECRET_KEY`, `SECRET_KEY_BASE`, `FRONTEND_URL`, `SOLID_QUEUE_IN_PUMA` (**required**: without it no job ever runs), `HONEYBADGER_API_KEY`, `ANTHROPIC_API_KEY`, `SMTP_HOST`, `SMTP_PORT` (`2587`), `SMTP_USER`, `SMTP_PASS`, `MAILER_FROM`, and `VAPID_PUBLIC_KEY` + `VAPID_PRIVATE_KEY` (**optional**: absent keys degrade push to `503 push_unavailable` and the digest to email-only; per-environment pairs, generated with `bin/rails push:vapid`; § Push notifications). `web` reads its own `.env.web` (`.env.web.example` is the template) for `ALLOWED_ORIGIN` alone: it has no code path that touches Postgres, Devise, or any of `api`'s secrets, so it no longer gets `env_file:`'d the whole root `.env` the way it did until this was caught (that shared file let a `web`-side compromise reach every backend secret; two separate files close it). Compose still reads the root `.env` automatically to resolve the `${DB_*}` interpolation inside `docker-compose.prod.yml` itself, independent of which service's `env_file:` points at it.

**`config/database.yml`'s `production` block takes those four keys discretely, the same shape as `development` and `test`, rather than a `DATABASE_URL`.** Compose briefly built that URL by interpolating the credentials into a connection string, which meant a password containing `:`, `@` or `/` either failed to parse or, worse, parsed into a URL aimed somewhere else, guarded by nothing but a comment telling the operator to pick a password without them. Passed as values there is nothing to escape and no constraint to remember. `DB_NAME`, `DB_HOST` and `DB_PORT` carry defaults; the two credentials use `ENV.fetch` with none, so a missing one stops the boot instead of silently connecting as some fallback identity.

**Ingress is a Cloudflare Tunnel (`cloudflared`), not a public listener.** The tunnel is outbound-only from the home machine to Cloudflare's edge, so there is no port to forward and no residential IP exposed; `cloudflared/config.yml` (gitignored: it names a real tunnel ID and points at that tunnel's credentials file, `cloudflared/config.yml.example` is the committed template) carries two ingress rules, `kk.chairulakmal.com` → `web:3000` and `kk-api.chairulakmal.com` → `api:8080` (replacing the old `api-production-4899.up.railway.app` for `/api-docs`), same two-origin shape the app already had, so nothing in `web/`'s CSP or `ALLOWED_ORIGIN` handling changed. **`kk-api`, not `api.kk`:** Cloudflare's default edge certificate covers only the apex plus one level of wildcard, and the two-level shape failed the TLS handshake outright during the verification window (confirmed via curl/openssl s_client: alert 552, no cert offered) before the real cutover ever used it. Because the origin has no other ingress path, the domain can now go **orange-cloud** (proxied through Cloudflare) rather than the grey-cloud (DNS-only) Railway's own Let's Encrypt ACME HTTP-01 challenge required; see notes/HISTORY.md for why that constraint existed and no longer does. **DNS cutover happened 2026-08-20**, after a parallel-then-switch verification window and a real-data restore drill (`TODO.md`): `cloudflared tunnel route dns --overwrite-dns` replaced the CNAME that pointed `kk.chairulakmal.com` at Railway, and Railway now serves nothing.

**Deploys are manual: `bin/deploy`.** There is no PaaS watch-path deploy to replace, so a merge to `main` is no longer itself a deploy; the operator runs `bin/deploy` (`git pull --ff-only`, `docker compose -f docker-compose.prod.yml build`, `up -d`) afterward. Migrations need no separate step: `api/bin/docker-entrypoint` already runs `db:prepare` on every boot of a `rails server` command (§ Production lessons below), so `up -d` alone brings the schema current.

### Network exposure, audited 2026-08-20

**The origin's reachable surface is exactly the two ingress hostnames above, and that holds at three independent layers.** A full network security audit ran against the running stack and the machine hosting it on the day of the DNS cutover; this section records the posture it verified, so that a later change which widens the surface shows up as a change rather than as a default nobody re-checked. The layers are listed in order of how much weight they carry, and the first one carries nearly all of it: it is a property of `docker-compose.prod.yml`, so it survives any firewall mistake, on any host.

- **No service publishes a host port.** All four containers sit on the single `internal` bridge, `cloudflared` dials *out* to Cloudflare's edge rather than accepting anything, and the backups runner reaches Postgres through `docker exec` rather than TCP (§ Backups). Measured rather than assumed: `ss -tlpn` filtered to non-loopback addresses returns **nothing at all**, so this machine binds no TCP service to a routable interface and a port scan has nothing to find, whatever the firewall is doing.
- **`ufw` is active: default deny incoming, allow outgoing, routed disabled.** `openssh-server` is not installed, so there is not even an SSH daemon sitting behind the deny rule.
- **The usual "Docker punches a hole straight through `ufw`" caveat does not apply here, and the reason is worth writing down rather than trusting the deny line.** Stock Docker Engine inserts DNAT rules into the nat table's `DOCKER` chain and accepts in `FORWARD`, which bypasses the `INPUT` chain `ufw` filters on, so a published port on stock Docker is reachable no matter what `ufw status` claims. This machine runs Docker Desktop for Linux, whose engine lives inside a VM: `iptables -t nat -L DOCKER` reports **no such chain**, there is no `/var/run/docker.sock` on the host at all (the socket is `~/.docker/desktop/docker.sock`), and a published port surfaces as an ordinary userspace bind, which `INPUT` does filter. So `ufw` genuinely covers Docker here. **That is a property of the container runtime, not of this app**: moving the stack to stock Docker Engine on another host reintroduces the bypass, and only the no-published-ports rule above would still hold, which is the reason that rule is the one to keep.
- **Containers run unprivileged.** `api` as uid 1000 (`rails`), `web` as uid 1000 (`node`), `cloudflared` as uid 65532, and `postgres`'s postmaster as `postgres` despite that image's root entrypoint. None is `privileged`, none adds a capability.
- **Every file holding a production secret is mode 600**: the root `.env`, `.env.web`, `cloudflared/config.yml` and its credentials JSON, `api/.env`, `api/config/master.key`, `web/.env.local`, and the runner's `.credentials` / `.runner` / `.env`. Four of those were **664** until this audit, all four created during the cutover, and the shape of that mistake is the part worth remembering: the older secrets already followed the convention, and the new deploy artifacts simply did not inherit it, because nothing enforces the mode. **The mode defends against other host users, not against the container.** Docker Desktop's file-sharing layer does not enforce DAC on a bind mount, which is why `cloudflared` still reads a 600 host file as uid 65532. That was verified directly rather than inferred, since a permission fix that broke the tunnel at the next restart would be worse than the exposure it closed.
- **The self-hosted runner's blast radius is bounded by one fact: `karirkalyan-backups` is private, and this repo's own workflows all run on `ubuntu-latest`.** A self-hosted runner attached to a *public* repo is the classic remote-code-execution hole, because a fork's pull request can propose the very workflow that runs on it. Nothing is attached to the public repo, so that path does not exist. What remains is not a network attack at all: push access to the private backups repo is arbitrary code execution on this machine, with `docker exec` into production Postgres. That is GitHub account hygiene, and it is why the runner is registered to that one repo rather than to the account (§ Backups).

**What the audit did not cover**, recorded so the gap is not mistaken for a clean result: Cloudflare's own edge configuration (WAF rules, rate limiting, Bot Fight Mode), which is the single layer standing in front of the origin that lives entirely outside this repo and therefore has no committed record anywhere. `TODO.md` holds it, along with the dependency finding this audit left open.

### Backups

**Backups stay on GitHub Actions; only where the job runs and how it reaches Postgres changed.** Railway's Hobby plan had no managed backups, so a GitHub-**hosted** runner in the private [`karirkalyan-backups`](https://github.com/chairulakmal/karirkalyan-backups) repo reached out to Railway's public Postgres nightly. A home Postgres behind a Tunnel has no route a GitHub-hosted runner can reach and shouldn't be given one, so the fix is a GitHub Actions **self-hosted runner**, installed as a systemd service on this same machine and registered to that repo alone. `check.yml` and `backup.yml` keep the same cron (05:15 JST), the same `scripts/fingerprint.sql`, the same `actions/upload-artifact` with **60-day retention**, and the same `state/fingerprint` / `state/last-backup` commit; `runs-on` moves from `ubuntu-latest` to the self-hosted label, and `psql`/`pg_dump` run via `docker exec` into the `postgres` service's own container rather than against a `DATABASE_URL` over TCP. That drops two things Railway's version needed and a self-hosted runner on the same host as the database doesn't: the per-run `sudo apt-get` dance that installed a version-matched client (the container already ships the exact matching binary, since it's the same image the server runs), and the `DATABASE_URL` secret itself (a local `docker exec` authenticates over the container's Unix socket, which Postgres trusts by default, needing no password). The dump is still never git-committed: it lives as the workflow's own artifact, and the repo only tracks the two small state files.

Two properties worth knowing, both load-bearing for the privacy page:

- **The dump is the full database**, which means **GitHub holds a copy of every resume**. That is why GitHub is one of the six named sub-processors in § Legal pages: the backup repo is private and the workflow is boring, which is exactly what makes it the disclosure a policy forgets.
- **It only dumps when the data changed.** The job fingerprints `users` / `applications` / `timeline_entries` (`count @ max(updated_at)`) and skips when the fingerprint matches the one the previous run committed, so `solid_queue` / `solid_cache` churn never triggers a dump.

A restore drill passed 2026-07-11: `db-dump-7` restored into a scratch Postgres 18.4 with zero errors, all 17 tables and every row intact. The drill steps live in the backups repo's README. The dump is deliberately **not** a live mirror on a second database: a second live database is HA machinery for an app whose actual need is an undo button.

### Production lessons, recorded so they are not relearned

- **No Thruster.** It fronted Puma on a different port, which is a double proxy this deployment has no reason to add either: `cloudflared` already terminates the connection at the edge of the container network, same role Railway's own proxy used to play.
- **`bin/docker-entrypoint`** ran `db:prepare` only when args matched `./bin/rails server` literally; switching to `bundle exec rails server -b 0.0.0.0 -p 8080` stopped matching and silently stopped running migrations. It now matches against `*"rails server"*`, and both `api/Dockerfile`'s `CMD` and this entrypoint still have to agree, whatever orchestrator is running the image.
- **DNSSEC** drifted once already after a Cloudflare key rotation (the DS record at the registrar no longer matched), causing SERVFAIL on validating resolvers, on this same zone. Disabled cleanly: remove the DS record at the registrar *first*, then disable DNSSEC in Cloudflare. Worth re-checking after any DNS change here, tunnel-related or not.
- **Per-IP throttles (`Rack::Attack`) key off a header a client could set directly on Railway** (§ Security), because Railway left the RFC-7239 `Forwarded:` header client-settable ahead of `X-Forwarded-For`. A Cloudflare Tunnel closes that off structurally: the origin has no ingress path except through Cloudflare's edge, so a client cannot reach `api`/`web` to inject either header itself. The `forwarded_priority` pin stays regardless, since removing a defense because a new deployment happens to make it redundant is how the next deployment reopens the hole silently.
...
**Decided 2026-08-03. There is no `v1.11.2` and there will not be one; the next tag this repo cuts is `2.0.0`.** The feature freeze (`TODO.md`) already meant every remaining `v1` release would be a patch, and a patch tag buys nothing here: releasing is not a distribution event, since `web/` is the only client, nobody installs a package, and **merging is already the release**, because a merge under a watch path deploys to Railway. So the tag was pure ceremony over a deploy that had already happened, and the ceremony has a cost the freeze made pointless to keep paying: a tag, a `gh release create`, and a `CHANGELOG` section rename per bug fix.

What it costs, stated plainly so it is a known trade rather than drift: **the deployed app now has no version number at all.** The rule above (the version lives in exactly one place, the git tag) still holds, so with no new tag the honest description of production is `v1.11.1` plus however many commits, and `web/package.json` remains a deliberate `0.0.0`. Nothing in the app displays a version, and no client negotiates one, so nothing breaks; but do not go looking for a number that names what is running, because there isn't one.

The mechanical test above is **not** retired with the tags. It still decides whether a change is allowed under the freeze at all, and it is what `2.0.0` will be measured against when the tagging resumes: the `positions` entity is still the item that forces the major. `CHANGELOG.md`'s newest section is titled for this, holding untagged deployed work rather than pretending to a version.
