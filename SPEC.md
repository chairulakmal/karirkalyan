...
**`upcoming` is the Upcoming agenda (`v1.11.0`),** the dated commitments merged into one chronological list: follow-ups and future interviews across the active applications, plus the user's `residence_expires_on` when it falls within `AGENDA_RESIDENCE_WINDOW_DAYS` (90). Each item carries a `type` tag (`follow_up` / `interview` / `residence`) and, except a `residence` item, the application it deep-links to; the list is capped at `AGENDA_LIMIT` (8). It is gated on `ACTIVE_STATES` for the same reason `isOverdue` is (a dated fact on a closed application is not actionable), and interviews are future-only so a finished one drops off. `web/` renders it as a dashboard section, never an `/upcoming` route, the same call the stat cards made against a dedicated page.
