...
Also: timezone correctness and keyboard/IME accessibility on the daily-driver surfaces. **`isOverdue()` now pins Tokyo** like `formatDate` beside it (§ Locale-sensitive formatting), ending a UTC-server-versus-JST-browser hydration mismatch that fired every morning before 09:00 JST; **the Upcoming agenda's residence row serialises `in_time_zone`**, not `Date#to_time`, so it stops emitting `+00:00` beside every other item's `+09:00` (§ Dashboard data); and on the list's filter bar (§ Dashboard, filters) **no control disables mid-fetch** (a disabled element cannot hold focus, so the search box dropped it on every submit) and **Enter is gated on `isComposing`**, so an IME confirming a kanji candidate no longer submits the form.
...
- [Frontend (`web/`)](#frontend-web) · [Tech stack](#frontend-tech-stack) · [Design system](#design-system) · [Auth flow](#auth-flow) · [Public pages](#public-pages) · [Legal pages](#legal-pages) · [Route guard](#route-guard) · [Caching](#caching-use-cache) · [Board view](#board-view) · [Pinned applications](#pinned-applications) · [Toast feedback](#toast-feedback) · [i18n](#i18n) · [Installable app](#installable-app)
...
`GhostRiskCard` re-ranks `at_risk` for display only: fewest `business_days_in_stage` first (the most recently touched of the at-risk set), capped at three with the rest behind a "Show more" toggle. The server's longest-silence ordering is unchanged on the wire; only the card's local render order and visible count differ from it.
...
`UpcomingAgenda` re-ranks the array for display only: closest to now first, by absolute distance (so a week-overdue follow-up outranks an interview three months out, the same way an overdue item reads as more urgent than a distant one), capped at three with the rest behind a "Show more" toggle. The server's chronological ordering is unchanged on the wire.
