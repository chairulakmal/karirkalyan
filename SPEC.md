...
**What the audits did not cover**, recorded so the gap is not mistaken for a clean result: Cloudflare's own edge configuration, meaning WAF rules, rate limiting and Bot Fight Mode. That is the one layer in front of the origin that lives entirely outside this repo. `TODO.md` holds it, along with the two hardening steps the 2026-09-17 container review left open: `api` still connects as the cluster superuser, and no container runs a read-only root filesystem.
