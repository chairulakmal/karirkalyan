...
Last synced against the code: **2026-09-01**.
...
- **Containers run unprivileged**: `api` and `web` as uid 1000, `cloudflared` as uid 1000 (set by `user:` in the compose file; the image's own default is 65532, see the mode-600 note below), `postgres`'s postmaster as `postgres`. None is `privileged`, none adds a capability.
