...
- [Backend (`api/`)](#backend-api) — [Tech stack](#backend-tech-stack) · [Data model](#data-model) · [State machine](#state-machine) · [Service layer](#service-layer) · [Query layer](#query-layer) · [API contract](#api-contract) · [Background jobs](#background-jobs) · [Mail](#mail) · [Security](#security) · [Passkeys (WebAuthn)](#passkeys-webauthn) · [Observability](#observability)
...
> **At a glance** · Four tables. `users` (Devise auth, `jti` for JWT revocation), `credentials` (WebAuthn passkeys — one row per enrolled authenticator), `applications` (the core FSM entity — `status`, plus `resume`/`cover_letter` as `bytea`), and `timeline_entries` (append-only audit log, one row per status change).
...
WebAuthn passkeys — one row per enrolled authenticator, zero for a password-only account. The table is purely additive (the previous image never writes to it), so it passes the versioning test for free. The full ceremony lives in § Passkeys; what the columns are for:
...
Four endpoints, two ceremonies (§ API contract for the list). Every challenge is a **single-use** entry in Solid Cache with a five-minute TTL, and verification **deletes the entry before verifying**, so a replayed or concurrent second attempt finds nothing:
...
`web/public/manifest.webmanifest` is a static file, served outside the locale tree and excluded from the proxy matcher (§ Routing internals). It declares `display: standalone`, and the shell behind the install is real: below `sm` the app's navigation moves to a bottom tab bar (§ The installed shell), long-pressing the launcher icon offers manifest `shortcuts` (§ Shortcuts), and a `monochrome` icon lets Android themed icons tint the monogram instead of dimming the full-colour plate (§ Icon purposes). What the installed app does *not* yet have is the rest of `v1.6.0` — passkey sign-in and push.
