      ...
      input_schema: {
        ...
        properties: {
          ...
          sponsorship: { type: "string", enum: [ "available", "unavailable", "" ],
                         description: "Work-visa sponsorship, meaning whether the employer will file for a visa or a change of status of residence: 'available' when the posting says it sponsors a visa or supports relocation to Japan; 'unavailable' ONLY when the posting says it does not sponsor. Empty string when not stated. A residency requirement is NOT an answer to this question: 'residents of Japan only', 'must already live in Japan', 'no relocation', or 'must have existing work authorization' mean the company will not hire from overseas, and say nothing about whether it sponsors a candidate already here, so return the empty string for those unless sponsorship is separately addressed." },
