...
CI is path-aware **per tree**: a docs commit pays for neither suite, a `web/` commit doesn't pay for the Rails suite, and an `api/` commit doesn't pay for the Next build. One asymmetry, deliberate: `web-ci`'s Playwright job boots the real Rails API, so an `api/`-only change still runs it, since it is the only job that tests the two halves against each other. It runs on pull requests too, so a contract break shows before the merge, but it is not a required check. Both workflows split into `changes` → `verify` → `gate`. **`gate` owns the required context name and must always run**: a required check that is skipped stays *expected* forever and blocks the merge, so path filtering lives in `changes`, never on the workflow trigger. If you rename a job, keep `gate`'s `name:` byte-identical to the ruleset's context string.
