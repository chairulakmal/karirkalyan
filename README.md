...
- Ghost prediction flags applications that have gone quiet: 15 working days after applying, 10 after a phone screen. **Working days**, so weekends, Japanese national holidays, Golden Week, Obon and the New Year shutdown are not held against a company that was closed. It reads the timeline audit trail directly, so there is no new column and no new table.
...
- The dashboard and board earn their keep off the same audit trail. Response rate, screening success rate, ghost rate, and average time-in-stage as stat cards (the first two sit together because the gap between them separates a targeting problem from a resume problem); an Upcoming agenda merges your follow-ups, interviews, and residence-expiry clock into one read of the week ahead (anything overdue stays on it however old, anything further out is one click away, and the residence clock is exempt because a Certificate of Eligibility takes 63 days); the dashboard opens on the applications still in play (archived ones live on the board's closed rail, out of the working list), with the long stage-chip row trimmed to the active stages plus Accepted (the graveyard stages stay reachable through the "All" preset, a shared `?status=` URL, and the board's closed rail); free-text search over company, role, and notes rides beside the structured filters, and all of it lives in the URL so a filtered or searched view is linkable and survives a reload; up to three applications can be pinned to the top of the list, held in the browser rather than the database because a pin is a transient attention marker and not a fact about the application, and a pin reorders only what is on screen, so it never contradicts the filters above it; the two candidate-side board columns (wishlist and draft) are triaged with a notes excerpt, the source it came from, and how long it has sat there, stalest-first; every status change can carry a note (a rejection's feedback, an offer's terms) on the timeline entry it belongs to; and cover-letter talking points are extracted as bullets from your resume against the posting (bullets, not a draft, because a generic AI letter is the wrong signal in this market). Every write speaks through one toast, including the reason a rejected optimistic board move snaps back.
...
- The product is bilingual, English and Japanese, and CI enforces catalog parity. A missing Japanese key lints, typechecks, and builds clean, so `npm run lint:i18n` diffs the two catalogs and fails the build on any key present in only one language. CI also proves the state-machine claim above: a companion check (`npm run lint:fsm`) fails the build if any TypeScript file hardcodes a set of FSM state names outside a justified allow-list, so "the rules exist in exactly one file" stays true rather than merely being trusted. Japanese text also breaks at phrase boundaries (文節) rather than mid-word: `word-break: auto-phrase` where the browser supports it, and server-side [BudouX](https://github.com/google/budoux) segmentation on the headings everywhere else.
