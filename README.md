...
- **AI pre-fill** — paste a job URL and Claude Haiku 4.5 extracts company/role/notes for review before saving. Server-side service, SSRF-guarded and rate-limited; reads Japanese postings natively. When a site refuses the fetcher, or serves a login wall instead of a posting, paste the text yourself and the same extraction runs on it — the fetch is the only step that can fail.
