...
- ダッシュボードとボードも、同じ監査証跡から価値を引き出します。返信率・音信不通率・平均滞在日数をスタットカードで表示し、フィルター状態は URL に保持されるので、絞り込んだビューはリンク共有でき、再読み込みでも維持されます。応募者側の2列（ウィッシュリスト・下書き）は、メモの抜粋・出所・滞在期間で古い順にトリアージされ、カバーレターの要点は履歴書と求人票を突き合わせて箇条書きで抽出します（下書きではなく箇条書き。この市場では汎用的なAIの文章は誤ったシグナルだからです）。
