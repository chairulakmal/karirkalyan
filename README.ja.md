...
| バックグラウンドジョブ | 冪等性キーパターン（at-least-once配信に対応）。Sidekiq無効化中のため`:async`アダプター（インプロセス）で実行 |
| メール送信 | ActionMailer + SMTP（Resend）。サインアップ時のウェルカムメールと日次フォローアップリマインダージョブ（Sidekiq無効化中は非稼動） |
| AI自動入力 | 求人URLを貼り付けると Claude Haiku 4.5 が企業名・職種・メモを抽出し、保存前に確認・編集できる。サーバーサイドのサービス層で実行、SSRF対策＋レート制限あり、日本語の求人もそのまま読み取る |
| キャッシュ | `:memory_store`（Sidekiq無効化に伴いRedis削除。Rack::Attackスロットルカウンターはプロセスごとに独立） |
...
- 非終端状態（`archived`を除く）はすべて `archived` へ遷移可能。タイムライン履歴を削除せずに通常ビューから非表示にできます。
...
- **バックエンド：** Rails 8 API-only、Ruby 3.4.9、PostgreSQL 16、Devise + devise-jwt
- **フロントエンド：** Next.js 16 App Router、Tailwind CSS
- **インフラ：** Docker Compose（ローカル）、Railway（本番）— マネージド PostgreSQL。Redis/Sidekiq は現在無効
