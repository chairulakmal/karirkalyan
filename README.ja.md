...
| バックグラウンドジョブ | Sidekiq + 冪等性キー（at-least-once配信に対応）。本番では専用の`sidekiq`サービスで実行 |
| メール送信 | ActionMailer + SMTP（Resend）。サインアップ時のウェルカムメールと、sidekiq-cronでスケジュールした日次フォローアップリマインダーを`deliver_later`で配信 |
| キャッシュ | 本番では Redis ベースの `Rails.cache`。ダッシュボード集計クエリの自己無効化キャッシュと Rack::Attack のスロットルストアを共有 |
...
- **インフラ：** Docker Compose（ローカル）、Railway（本番）— `api`（Puma）と `sidekiq`（ワーカー）を分離、マネージド PostgreSQL + Redis
