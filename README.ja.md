...
- ダッシュボードとボードも、同じ監査証跡から価値を引き出します。返信率・音信不通率・平均滞在日数をスタットカードで表示し、フォローアップ・面接・在留期限を1つの時系列にまとめた「予定」欄も置きます。ダッシュボードはまだ進行中の応募だけを最初に表示し（アーカイブ済みはボードのクローズド欄に退避し、作業リストには出しません）、長いステータスチップの列は進行中のステータスと「内定承諾」だけに絞り込みます（絞られた終了ステータスは「すべて」プリセット、共有された `?status=` の URL、ボードのクローズド欄から変わらず辿れます）。会社名・職種・メモを対象とした自由文検索が構造化フィルターの隣に並び、いずれも URL に保持されるので、絞り込みや検索したビューはリンク共有でき、再読み込みでも維持されます。応募者側の2列（ウィッシュリスト・下書き）は、メモの抜粋・出所・滞在期間で古い順にトリアージされます。どのステータス変更にも、その変更が属するタイムラインエントリにメモ（不採用のフィードバック、オファーの条件など）を残せます。カバーレターの要点は履歴書と求人票を突き合わせて箇条書きで抽出します（下書きではなく箇条書き。この市場では汎用的なAIの文章は誤ったシグナルだからです）。すべての書き込みは1つのトーストで通知され、ボードの楽観的な移動が差し戻された理由もそこで伝えられます。
