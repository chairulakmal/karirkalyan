...
| メール送信 | ActionMailer + SMTP（Resend）。サインアップ時のウェルカムメールと毎日8:15 JSTのフォローアップリマインダー（Solid Queue定期タスク） |
...
web/
  ...
  app/(app)/board/board.tsx               ← カンバンボード。ドラッグ＝遷移、可否はAPIから取得
```
...
- **インフラ：** Docker Compose（ローカル）、Railway（本番）— マネージド PostgreSQL。Solid Queue + Solid Cache を同一Postgresで運用（Redis不要）
