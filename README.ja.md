...
| メール送信 | ActionMailer + SMTP（Resend）。アカウント作成時のウェルカムメールと、毎日8:15 JSTに送る**ダイジェスト**（Solid Queue定期タスク）。応募ごとに1通ではなく、1日1通にまとめる |
| 新規登録 | 受け付けていない。サインアップ画面は存在しない — このアプリは履歴書を預かる。作者以外の個人情報を預かる設計には、あえてしていない。上記のデモアカウントでサインインしてほしい。デモがそのまま製品版である。実アカウントは運用者がサーバー上で作成する（`bin/rails users:create`）。登録を閉じたことでパスワード再設定メールの経路も無くなったため、パスワードを忘れた場合は `bin/rails users:set_password` — JWT の `jti` が更新され、そのユーザーは全端末からサインアウトされる |
| 法務ページ | [`/privacy`](https://kk.chairulakmal.com/privacy) と [`/terms`](https://kk.chairulakmal.com/terms)（日英両対応）。ひな形の模倣ではなく、実装どおりの事実を書いている：委託先5社を実名で列挙、機能的クッキー2つ、アナリティクスなし。そして「自分で削除できるボタン」は約束しない — 存在しないからだ。削除依頼は運用者へのメールで、運用者が `DELETE /api/v1/auth/account` を実行する |
