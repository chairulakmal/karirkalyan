...
Rails 8 API と Next.js 16 のカンバンボードで構成した求人応募トラッカーです。ステータス変更はすべてサーバー側で強制される有限状態機械を通り、クライアントは遷移ルールを自前で持たず、どの移動が合法かを API に問い合わせます。以下、ライブデモ、ハイライト、技術スタック、ローカルでの動かし方、テストと CI の順に説明します。設計判断の解説は [ARCHITECTURE.md](ARCHITECTURE.md)（英語）にあります。
...
- 日本市場レイヤーは、汎用トラッカーが無視するものを記録します。各応募は応募チャネル（直接・エージェント・リファラル）を持ち、二重推薦の前にアプリが警告します。最初に企業へ推薦したエージェントがその候補をおよそ12〜18ヶ月「保有」し、別ルートで入社しても紹介料は保有者に支払われるため、期間中の二重推薦は既存ツールが黙って見過ごす実害です。年収は1つの数字ではなく構造（保証月数と業績連動賞与の月数）として保存し、求人の日本語要件（JLPT N1/N2・ビジネス・日常会話・不要）は第一級のフィルターになり、求人票の本文はプレフィル時にスナップショットされるため、求人が取り下げられても面接準備の材料が残ります。これらはすべて企業名や職種を読み取るのと同じ AI パスで抽出されるため、記録にかかるのはデータ入力ではなく確認だけです。
- フォローアップのデイリーダイジェストは JST 8:15 に送信され、週末と日本の祝日（年末年始、ゴールデンウィーク、お盆を含む）はスキップします。スキップされたリマインダーは破棄されず翌営業日に繰り延べられ、ちょうど1回だけ送信されます。冪等キーが送信日ではなくフォローアップ予定日から導出されているためです。`/settings` で通知を有効にすると、同じダイジェストがインストール済みアプリへのプッシュ通知としても届きます。バッチは1つ、チャネルは2つで、プッシュの失敗はメールを再送することなくリトライされます。
- プロダクトは英語と日本語のバイリンガルで、カタログの整合性は CI が強制します。日本語キーの欠落は lint も型検査もビルドも素通りするため、`npm run lint:i18n` が2つのカタログを比較し、片方の言語にしか存在しないキーがあればビルドを失敗させます。さらに日本語テキストは単語の途中ではなく文節の区切りで改行されます。対応ブラウザでは `word-break: auto-phrase`、それ以外では見出しに対するサーバーサイドの [BudouX](https://github.com/google/budoux) 分かち書きで実現しています。
...
- サインインはパスワードに加えてパスキーでも行えます。WebAuthn は gem 任せにせず Devise に手で組み込みました（`webauthn` gem）。ディスカバラブルクレデンシャルによりユーザー名の入力は不要で、認証器の制限を設けないため Proton Pass のようなサードパーティプロバイダーもそのまま使えます。デスクトップで作成したパスキーはパスワードマネージャー経由でスマートフォンに同期します。検証済みのアサーションはパスワードサインインと同じ、有効期限1日・JTI 失効可能な JWT を発行するため、サインアウトはこれまで通りすべてのデバイスを失効させます。パスワードフォームは恒久的なフォールバックとして残り、パスキーの登録は `/settings` から行います。
...
- インストール後は、フレームに入ったサイトではなくアプリとして振る舞います。スマートフォン幅ではジェスチャーバーを避ける余白を持つボトムタブバーが表示され、ランチャーアイコンの長押しで「新規応募」と「カンバン」のショートカットが開き、`monochrome` マニフェストアイコンによって Android のテーマアイコンがフルカラーのプレートを暗くする代わりにモノグラムを着色できます。
