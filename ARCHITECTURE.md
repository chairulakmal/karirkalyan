...
This file is a short technical overview of KarirKalyan. Its most important point: every claim in it can be checked directly in the code, because a file path is given next to almost every sentence. [SPEC.md](SPEC.md) is the full technical specification; this file is the shorter, simpler version. It covers eight decisions that shape most of the codebase. Each one is explained the same way: the choice, the reason for it, and what was given up to make it. The first four are a state machine written in plain Ruby; one database transaction that writes every status change; ghost risk calculated from the audit log instead of stored in a column; and a reminder job that delays messages but never loses them. The last four are an automatic check that keeps the English and Japanese text in sync; one PostgreSQL database for the whole app; a JWT that never reaches the browser's JavaScript; and a move from Railway to self-hosted Docker and Cloudflare Tunnel. A JWT (JSON Web Token) is a signed piece of text that proves who a user is.
...
A gem like that would add callbacks, guards, and its own small language for describing events. This app does not need any of that. The whole rule list fits in one file that reads from top to bottom. One rule is not a row in that list: any non-final state can move to `archived`. That rule is a single guard clause (a short check) inside `assert_transition!`. The trade-off is that side effects of a status change must be written by hand in the service layer instead of through the gem's event system. That fits how the rest of this codebase is already organized.
...
The diagram leaves out a few rules for clarity. Any non-final state can move to `archived`. The four pipeline states (`applied` through `final_round`) can move to `withdrawn`. From `offer`, a candidate who says no uses `declined`, not `withdrawn`. Before `applied` there is nothing to withdraw from yet, so an application abandoned that early goes to `archived` instead. Also, `rejected`, `ghosted`, and `withdrawn` can each move back to `applied`, because a recruiter sometimes cancels a rejection, and a company that stopped answering sometimes returns. Only `accepted`, `declined`, and `archived` are truly final.

Creating a new application is different from transitioning one. A new application can start in `wishlist`, `draft`, or `applied` (the `ENTRY_STATES`). Every state after those three is reachable only through a real transition, so the audit trail always records every actual change.
...
Two tabs, or two people, can edit the same application at the same time. The app handles this optimistically: it assumes that conflicts are rare, and it checks for one only at the moment of saving. The transition endpoint ([`api/app/controllers/api/v1/applications_controller.rb`](api/app/controllers/api/v1/applications_controller.rb), `#transition`) sends the client's `lock_version` number along with the save. If that number is out of date, the save raises `ActiveRecord::StaleObjectError`, which [`api/app/controllers/application_controller.rb`](api/app/controllers/application_controller.rb) turns into a `409 Conflict` response with the code `stale_record`. On the board page, dragging a card moves it on screen right away, then snaps it back if the server answers `409` ([`web/app/[locale]/(app)/board/board.tsx`](web/app/%5Blocale%5D/%28app%29/board/board.tsx)).

The trade-off: if two tabs save the same application at the same time, the tab that loses must reload the page and make its edit again. The alternative is worse. Without this check, the second save would overwrite the first with no warning, and real work would disappear.

## Ghost risk is calculated, not stored

[`api/app/queries/applications/ghost_risk_query.rb`](api/app/queries/applications/ghost_risk_query.rb) flags applications where the company has stayed silent longer than expected for its current stage. It works out when the current stage began using only the `timeline_entries` table: the most recent row that changed the status, or `applied_at`, or `created_at` as a fallback, since creating an application writes no timeline row. A follow-up reminder also writes a timeline row, with the same status before and after. That row is not a stage change, so the query skips it (`TimelineEntry::STAGE_CHANGE_SQL`); otherwise every reminder would restart the count. No new database column or table is needed. The audit rows the transition service already writes are the entire dataset this feature reads.
...
The app sends one follow-up reminder email per user per day, never one email per application. The schedule is set in [`api/config/recurring.yml`](api/config/recurring.yml) as `15 8 * * * Asia/Tokyo`, which means 8:15 AM Tokyo time, every day. It runs through Solid Queue, a background job system that stores its own data in the same PostgreSQL database used for everything else. The job runner runs inside the same process as the web server, Puma (`plugin :solid_queue` in [`api/config/puma.rb`](api/config/puma.rb)), so there is no separate worker service to run or manage.

[`api/app/jobs/follow_up_reminder_job.rb`](api/app/jobs/follow_up_reminder_job.rb) first asks [`api/app/lib/japan_calendar.rb`](api/app/lib/japan_calendar.rb) whether a company would realistically answer a reminder sent today. If not, the job holds and tries again on the next valid day. National holidays come from the `holidays` gem, for two reasons. Two of these holidays move every year with the equinoxes. And by law, a holiday that falls on a Sunday also makes the following Monday a holiday (振替休日, substitute holiday). New Year, Golden Week, and Obon are added separately as fixed date ranges, because they are not legal holidays and the gem does not know about them. The real question is never whether a government office is open. It is whether anyone at the company will actually read and answer an email.
...
The trade-off: the exactly-once guarantee comes from a unique constraint in the database, not from the scheduler. This is also why it is safe when the scheduler runs the same job twice. The database is what stops the second email, so the scheduler does not have to be perfect.
...
This choice comes with deliberate limits. Storing files as `bytea` instead of using a separate file-storage service only works because uploads are capped at 1 MB each and applications are capped at 200 per account. A rate limit alone cannot bound total storage, because every time window resets, so the hard cap on application count is what actually does that job. The files also stay out of every read that loads many rows at once: `Application.without_blobs` leaves out the PDF columns, and the account export reads the files ten applications at a time into a temporary file. Without that, one page of the list could load up to 200 MB of PDFs into a 1 GB container. Running background jobs inside the same process as the web server only works because the workload is small. It is four tasks: one daily follow-up reminder, one daily interview and residence reminder, one hourly cleanup task, and one hourly reset of the shared demo account ([`api/app/jobs/demo_reset_job.rb`](api/app/jobs/demo_reset_job.rb)) back to its starting data.

## The JWT never reaches the browser

Signing in returns a JWT inside a response header. A Next.js route handler ([`web/app/api/auth/session/route.ts`](web/app/api/auth/session/route.ts)) captures that token and stores it in an `httpOnly` cookie, a cookie that JavaScript running in the browser cannot read. Every API call after that happens server-side, through [`web/app/lib/api.ts`](web/app/lib/api.ts), so a cross-site scripting bug (XSS, a security flaw where an attacker injects unwanted JavaScript into a page) can never steal the token. This is the main reason the frontend uses Next.js instead of a single-page application, a website that runs entirely in the browser. A single-page application has no server layer, so a server layer would have to be built anyway, only to set this cookie.

Revoking access means canceling a token before it expires. This uses devise-jwt's `JTIMatcher` strategy: each user has one `jti` (JWT ID) column, and signing out changes that column to a new value. Every token issued before that moment becomes invalid at once. The trade-off: each user can hold only one active session, tokens expire after one day, and there is no refresh flow. Signing out on one device signs the user out on every device, and signing in again is the only way to get a new token.

Route protection lives in [`web/proxy.ts`](web/proxy.ts). Next.js 16 renamed `middleware.ts` to `proxy.ts`, so the old filename is silently ignored. The same file also builds a fresh CSP nonce on every request. A nonce is a security value that is unique to each page load, and the browser runs only the scripts that carry it. This is why every page is rendered again for each request instead of being served as a cached static file.

## Deployment moved from Railway to self-hosted Docker and Cloudflare Tunnel

As of August 20, 2026, KarirKalyan no longer runs on Railway, a cloud hosting platform. It runs on self-hosted Docker containers on the author's own mini PC, and visitors reach it through a Cloudflare Tunnel. The move removed a running cost of about ten US dollars a month, for an app that has exactly one user. A mini PC uses little power, so the electricity cost is low. This setup is temporary. The planned rewrite to Hono and Nuxt on Cloudflare's free tier is the long-term $0 host, and it will replace these containers ([`TODO.md`](TODO.md)). Four containers share one internal Docker network, defined in the root [`docker-compose.prod.yml`](docker-compose.prod.yml): `postgres`, `api`, `web`, and `cloudflared`.

No container opens a port directly to the internet. `cloudflared` is the only entrance: it opens an outbound connection to Cloudflare, so there is no inbound port for an attacker to reach, and a visitor never learns the home IP address. Requests the server makes itself, such as fetching a job posting for pre-fill, do leave from the home IP. That is why the shared demo account cannot pre-fill from a URL. Traffic reaches `api` and `web` only by their internal Docker names. The domain, `FRONTEND_URL`, and every other environment variable stayed the same as they were on Railway. Only two things changed: who runs the containers, and how traffic reaches them.

The move succeeded. The app has been live at its production domain since the switch. Both the `api` and `web` containers must pass a health check before any traffic reaches them ([`api/Dockerfile`](api/Dockerfile), [`web/Dockerfile`](web/Dockerfile)). Deployment is now a manual step: `bin/deploy` pulls the latest code, rebuilds, and restarts. It no longer happens automatically on every merge to `main`.
