...
This choice comes with deliberate limits. Storing files as `bytea` instead of using a separate file-storage service only works because uploads are capped at 1 MB each and applications are capped at 200 per account. A rate limit alone cannot bound total storage, because every time window resets, so the hard cap on application count is what actually does that job. Running background jobs inside the same process as the web server only works because the workload is small. It is four tasks ([`api/config/recurring.yml`](api/config/recurring.yml)): the daily reminder email above, a second daily job that pushes interview and residence-expiry reminders ([`api/app/jobs/interview_reminder_job.rb`](api/app/jobs/interview_reminder_job.rb)) at 08:00 JST, one hourly cleanup task, and one hourly reset of the shared demo account ([`api/app/jobs/demo_reset_job.rb`](api/app/jobs/demo_reset_job.rb)) back to its starting data.
