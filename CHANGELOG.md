...
- **`.github/dependabot.yml` is commented out** (2026-07-22), a one-line reactivation away. Shipped active in `v1.11.0`, its first monthly run opened **nine** PRs simultaneously: a grouped `api` and `web` pair, four majors held out of the groups (typescript 7, eslint 10, `@types/node` 26, holidays 11), and three Actions bumps. That is precisely the required-check and Railway-deploy noise the config was meant to spare the two required checks, arriving all at once rather than trickling. The seven green PRs were merged; the two build-breaking majors (typescript 7, eslint 10) were left open, since adopting them needs code work, not a merge. Paused until we choose to turn it back on, with the majors it surfaced tracked as their own decision rather than by this file.
