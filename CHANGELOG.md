...
The history: shipped work, newest first (`v1.11.0`, tagged 2026-07-28, back through `v1.0.0`). One unreleased head sits above it: **`v1.11.1`**, the patch collecting the bug fixes a post-merge review of the `v1.11.0` code turned up: a list search that never reached the API, plus the timezone and keyboard/IME set. `v1.11.0`, the daily-driver minor, landed across two PRs (#83, which squash-landed the whole `batch/2026-07-22` at once, and #93) and it also carries three fixes that were scoped as `v1.11.1` work but reached `main` before the tag did: they are recorded in the `v1.11.0` section below, where the tag that shipped them puts them, not in the patch above it. `v1.11.0` absorbed the daily-driver work `TODO.md` had briefly split into `v1.12.0`/`v1.12.1`, so that split is gone. **`v1.9.0` carries no tag of its own** (decided 2026-07-22): it and `v1.10.0` were built in one batched PR (`feat/v1.9-and-v1.10`, #82), the `v1.10.0` tag already points at the commit that includes the `v1.9.0` work, so a separate tag would relabel already-released code; its section stays below for the record. Branch/PR names note where each change landed, and each release since `v1.3.1` names its patch/minor level against the versioning policy's mechanical test. The most important section is the one that is not a release: § Decisions records the settled decisions-not-to-build, so that [`TODO.md`](TODO.md) (where open work lives) can stay plan-only without the reasoning getting lost.

---

## v1.11.1 (unreleased)

The patch above the daily-driver batch: correctness fixes a review of the merged `v1.11.0` and the shipped `v1.10.0` code turned up. Most of it is of one shape, timezone correctness and keyboard/IME accessibility on the surfaces the daily driver just grew; the exception leads the section, because a filter that returns everything and reports success is a different order of problem from a badge with the wrong colour. Those bug fixes are the ones scoped in [`TODO.md`](TODO.md) § v1.11.1, landing here entry by entry as each fix is written, so nothing in this section claims a fix the code does not yet hold. A dependency and lockfile chore rides along at the end, below the fixes because it is the least user-visible thing here; it is not in `TODO.md`'s list and does not need to be, since that list is the patch's *bug* scope. A patch by the mechanical test: no migration, no `v1` contract broken, no new capability, each item makes something the app already does work correctly. Three further fixes `TODO.md` had scoped into this patch (the sign-out destination, the E2E smoke-test locator, the Dependabot pause) reached `main` before the `v1.11.0` tag was cut, so they shipped inside that tag and are recorded in its section below rather than here.

### Fixed: the list search returned every application, and said it had filtered

- **The BFF route handler never forwarded `q` to the API, so every *typed* search silently returned unfiltered results.** `web/app/api/applications/route.ts` re-enumerated the list's query params by hand, `q` shipped in `v1.11.0`, and the handler was not taught about it: the client sent `?q=Mercari`, the handler built an upstream query without it, `ListQuery` received `q: nil` and returned the whole page. Nothing signalled the miss. The box still held the term, the URL still said `?q=`, "Clear filters" still lit up, and the list simply showed everything. The inconsistency is what made it hard to see: `dashboard/page.tsx` *did* pass `q` on the first paint, so **reloading a `?q=` URL searched correctly** and only typing was broken; and once a working server-side search was on screen, the next chip toggle re-fetched through the BFF and quietly replaced it with unfiltered rows. `SPEC.md` § Query layer argues at length that `q` must be a server param, because a client-side scan would miss matches on unloaded pages; the shipped behaviour was worse than the option that paragraph rejects, since it missed matches on loaded pages too. Spec and code disagreed and the code was the bug, so `SPEC.md` is unchanged.
- **Fixed at the class, not the instance.** The param allowlist now lives once, in a new `app/lib/list-params.ts`, and the handler iterates it; adding a filter to the API is one edit in one file rather than one edit plus a memory. A hand-copied allowlist was invisible to `tsc`, to rswag, and to the docs sweep, which is why three other surfaces documented `q` correctly while this one dropped it.
- **The missing test seam is opened with it.** A route handler is a plain `Request → Response` function, unit-testable with no server and no browser, and `app/api/**/route.ts` had no tests of any kind. There are now nine: `app/lib/list-params.test.ts` asserts every param in the contract survives the hop, and `app/api/applications/route.test.ts` drives the handler itself with `apiFetch` stubbed, covering the forwarding, the success passthrough, and that an upstream failure keeps its status instead of being masked as a 200.


### Fixed: "today" was the ambient clock, not Tokyo

- **`isOverdue()` derived today's date from `new Date().getFullYear()/getMonth()/getDate()`, then string-compared it against a date the API serialises in Tokyo.** The `web` service runs UTC on Railway (no `TZ` is set) and the browser runs JST, so **between 00:00 and 09:00 JST the two disagreed about what day it is**: the server rendered a genuinely-overdue follow-up as not overdue (saffron, a bare date) and the browser re-rendered it overdue (danger, "Overdue"), a different i18n key *and* a different className for the same row, which React 19 reports as a hydration mismatch. It fired on the dashboard list, the detail page, and the new Upcoming agenda, every morning, which is exactly when this app gets opened. Today is now `Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Tokyo" })`, the zone its sibling `formatDate` already pinned (and whose own comment had warned about this disagreement). Covered by a new `app/lib/format.test.ts` that fakes an instant inside the broken window. `todayISO()` in the new-application form is a deliberate near-twin and is left alone: it seeds a date input, not a comparison.
- **The Upcoming agenda's residence row serialised the wrong offset.** `expiry.to_time` resolves a bare `date` column in the container's system zone (UTC on Railway), so that one item emitted `+00:00` while every other agenda item, coming from a `datetime` column, emitted `+09:00`. User-invisible today (only interviews render a time), and fixed while in the neighbourhood because the family of bug is the same one above: `expiry.in_time_zone`. The test gap that hid it is closed with it, since `dashboard_spec` asserted the agenda's `type` and `company` but never its `at`.

### Fixed: the list's filter bar was unusable from the keyboard, and hostile to an IME

- **No filter control disables while a page is in flight any more.** The search box carried `disabled={loading}`, so submitting a search disabled the input, the browser blurred it to `<body>`, and focus was never restored: consecutive searches cost a keyboard or screen-reader user a full tab back each time. This is the exact failure the same file had already solved on its chip `<fieldset>`, whose comment spells out why it is deliberately not `disabled`; the new input contradicted the file's own rule. The three filter dropdowns carried the same pre-existing flag and are folded into the fix. Blocking the handler is what stands in for it, as before: `applyFilters` early-returns on `loading`. "Load more" and "Clear filters" keep their `disabled`, being a busy indicator and a control that unmounts, not filters.
- **Enter no longer submits the search mid-IME-composition.** A Japanese user pressing Enter to confirm a kanji candidate could have that keystroke leak through as a form submit, searching half-composed text: the canonical Japanese-input footgun, on the market this app targets. The submit is now gated on `e.nativeEvent.isComposing` (the native event, since React does not surface it on the synthetic one).
### Chore: the lockfile disagreed with itself about Bundler

- **`api/Gemfile.lock` recorded two different Bundler versions and rewrote itself on every `bundle` command.** Its `CHECKSUMS` block named `bundler (4.0.16)` while `BUNDLED WITH` still said `4.0.12`, so any local `bundle` invocation (even `bundle check`) reset the checksum line to the installed version and left the working tree dirty. Both lines now read **4.0.17**, via `bundle update --bundler`. Nothing else moved: `bundle check` is satisfied, the suite is 506 green under it, and `bundler-audit` reports no vulnerabilities. **How the split happened, since the first version of this entry got it wrong and the wrong version would cause a recurrence:** `rails` depends on `bundler (>= 1.15.0)` (and `bundler-audit` on `bundler (>= 1.2.0)`), so Bundler is part of the resolution and earns a `CHECKSUMS` self-entry. That self-entry was **deleted by hand** in `dad7390` (2026-07-20) as apparently stale, at which point `CHECKSUMS` and `BUNDLED WITH` did agree, both absent and 4.0.12 respectively. Dependabot's next lockfile regeneration, the `api` minor/patch group (#87), then **re-added** it at the version *its* resolver ran, 4.0.16, leaving `BUNDLED WITH` at 4.0.12, which Dependabot does not manage. Two lessons, and the first is the one that bites: **do not delete the Bundler self-entry**, it comes back at whatever version regenerated the lockfile. And when Dependabot is un-paused, a Bundler move from it needs `bundle update --bundler` behind it, or the two drift apart again. The Docker build is unaffected: `ruby:3.4.9-slim` ships Bundler 2.6.9, and `bundle install` auto-switches to the `BUNDLED WITH` version. `api/Dockerfile` has never pinned Bundler explicitly (its only bundler-related lines are `BUNDLE_*` env and `RUN bundle install`), so nothing there needed to change.
- **The ten gem updates that accumulated while Dependabot was paused, taken in one pass** (`bundle update`, all inside the existing `Gemfile` constraints): `anthropic` 1.56.1 → 1.59.0, `solid_queue` 1.4.0 → 1.5.0, `rbs` 4.0.3 → 4.1.0, `raabro` 1.4.0 → 1.5.0, `net-imap` 0.6.4.1 → 0.6.6, `erb` 6.0.5 → 6.0.6, `csv` 3.3.5 → 3.3.6, `msgpack` 1.8.3 → 1.8.4, `concurrent-ruby` 1.3.7 → 1.3.8, `simplecov` 1.0.2 → 1.0.3. All minor or patch, and the two whose public API this app calls directly were checked rather than assumed: **`solid_queue` 1.5.0 needs no migration here** (its install template's `queue_schema.rb` is byte-identical between 1.4.0 and 1.5.0, and its eleven tables and their columns match `db/schema.rb` exactly, so the release stays schema-clean), and **`anthropic` 1.56.1 → 1.59.0 is additive** (a new model id, a new `model_context_window_exceeded` stop reason, new tool-change events), with `Anthropic::Client.new` and `client.messages.create` unchanged, which is the whole of this app's surface on that gem.
- **`solid_queue` 1.5.0 also changes how a zone-less recurring schedule resolves, which the schema check would not have caught.** Its `UPGRADING.md` is the file that says so: a schedule with no explicit time zone is now read in `config.time_zone` rather than the system zone. This app sets `config.time_zone = "Tokyo"` and runs on Railway with no `TZ`, i.e. system UTC, so it is exactly the configuration the note is addressed to. **All four tasks are unaffected, for two different reasons.** `follow_up_reminders` and `interview_reminders` name `Asia/Tokyo` in the cron string, so the default never applies to them. `clear_solid_queue_finished_jobs` and `reset_demo_account` are zone-less (`every hour at minute 12` / `at minute 42`), so their resolution *did* change, but JST is a whole-hour offset and an hourly-at-minute-N schedule maps to the same set of instants in UTC and in `Asia/Tokyo`. That is a property of the offset, not of the schedule: a half-hour zone, or a daily zone-less task, would have moved. `config/recurring.yml`'s header comment claimed the timezone was explicit in every cron expression, which was true of two tasks out of four; it now says which are which, so the next zone-less task added does not inherit a false assurance. **The procedural lesson:** for a gem that owns a runtime behaviour (scheduling, retries, serialization), `UPGRADING.md` is the first file to read and the schema is the second question.
...
### Also in this tag: three fixes scoped as `v1.11.1`

These were written against the open patch and reached `main` before the tag was cut, so the tag ships them; `TODO.md` § v1.11.1 keeps the remaining four bug fixes.

- **Signing out pushed to `/sign-in`; it now lands on `/`, the marketing page** *(fix, #94)*. Dropping someone on the sign-in form immediately after they chose to leave says "you were kicked out, sign back in", which is the expired-session bounce's message (`/api/auth/expired` → `/sign-in?expired=1`), not a deliberate sign-out's. The two paths had been telling the same story for different events. The marketing page keeps sign-in one click away for the case that actually wants it, signing out of the wrong account. Nothing about the session itself changed: `DELETE /api/auth/session` still rotates the JTI on the Rails side and clears both cookies before the redirect, and because `/` is a `PUBLIC_PATH` in `proxy.ts` the now-tokenless visitor is not bounced back to `/dashboard`. Recorded in `SPEC.md` § Auth flow, which had never named the destination. The only item in the patch that came from the app's own user rather than from a review pass.
- **The E2E smoke test's ambiguous `Company` locator** *(fix, test-only)*. `web/e2e/smoke.spec.ts` located the company field with `getByLabel("Company")`, a substring match that began resolving to two elements once `v1.9.0` added the `company_timezone` `<select>` (its label, "Company timezone", contains "Company"). Playwright strict mode refused the ambiguous locator, so the **Playwright E2E job had been red on `main` since #82** and nobody was stopped: the job is path-filtered (a docs-only push skips it and reports green, which masked the breakage) and it is not one of the two required checks, so a red run never blocked a merge. The locator is now `getByLabel("Company", { exact: true })` at both call sites, binding it to the input alone. No product change and no `web/` runtime touched; the point is the signal, since a globally-red E2E run cannot catch a real regression.
- **Dependabot paused after its first run opened nine PRs at once** *(chore)*. `.github/dependabot.yml` is commented out (2026-07-22), a one-line reactivation away. Shipped active earlier in this same release, its first monthly run opened **nine** PRs simultaneously: a grouped `api` and `web` pair, four majors held out of the groups (typescript 7, eslint 10, `@types/node` 26, holidays 11), and three Actions bumps. That is precisely the required-check and Railway-deploy noise the config was meant to spare us, arriving all at once rather than trickling. The seven green PRs were merged; the two build-breaking majors (typescript 7, eslint 10) were closed, since adopting them needs upstream ecosystem work, not a merge. Paused until we choose to turn it back on.
